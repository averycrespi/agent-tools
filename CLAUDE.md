...
GitHub Actions selects affected tools on PRs and runs the full tool set on `main`, manual runs, and a weekly schedule. Go lint/unit, integration, E2E, and blocking vulnerability checks are tool-scoped; Gateway lint runs independently of correctness, Gateway's harness/material and temporary-runner leaves have separate jobs, and sandbox-manager retains its macOS unit tests. Gateway's source-derived suite planner keeps component, harness, and material coverage out of its dependency-light unit path without omitting it from CI. Formatting and CI classifier/gate tests always run. The stable `Required` gate permits only expected skips. See README's CI section for path invalidation and branch-protection migration.

Keep `.github/workflows/ci.yml`, `.github/actions/go-cache/action.yml`, and `.github/scripts/ci.py` aligned with root Makefile targets. Go caches are tool/role-owned with workspace, dependency, linter, toolchain, and platform invalidation plus fresh run/attempt save keys; restoring cached build material never supplies exact-head correctness evidence. CI reads `TOOLS`, `INTEGRATION_TOOLS`, and `E2E_TOOLS` as literal lists; update the inventory assertion in `.github/scripts/ci_test.py` when the tool set changes. Run `make test-ci` (Python 3, no third-party packages) when changing CI. Do not invoke Gateway's `test` aggregate alongside its integration leaf; root `make check` followed by root `test-integration` would also repeat Gateway coverage. Non-Gateway integration owners use the single `integration` build tag and `TestIntegration*` namespace; their explicit packages and names are checked by `.github/scripts/integration_test.py`. Keep helpers beside their packages and give new runnable examples/fuzz seeds an explicit owner rather than letting them miss the selector. `TEST_JSON=1` enables Gateway Go-event output through Make without changing test selection.
