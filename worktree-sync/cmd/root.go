...
func NewWTS(controller app.Controller) *cobra.Command {
	...
	for _, spec := range []struct{ name, short, action string }{
		{"path", "Print the configuration file path", "config.path"}, {"edit", "Edit configuration and validate after saving", "config.edit"}, {"validate", "Validate the current configuration", "config.validate"}, {"refresh", "Migrate configuration and add missing defaults", "config.refresh"},
	} {
