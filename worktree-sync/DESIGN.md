...
Configuration is versioned JSON under the XDG `worktree-sync` namespace. The registry stores canonical primary roots, canonical common Git directories, stable user-selected repository IDs, one canonical worktree creation root, and a complete canonical allowlist. Global `default_worktree_creation_root` and `default_allowed_worktree_roots` seed newly registered repositories; default changes never alter existing registrations. `--worktree-root` replaces the default creation root, and repeatable `--allowed-worktree-root` values replace the default additional allowlist for one registration. When no creation root is selected, registration creates and uses the private XDG data root. The creation root is automatically added to the repository allowlist, and allowed-root ordering has no creation semantics. Registration accepts only a primary non-bare worktree; a linked worktree is not a valid base.
