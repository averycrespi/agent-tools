...
func run(controller Controller, action string, options func(*cobra.Command) map[string]any) func(*cobra.Command, []string) error {
	return func(cmd *cobra.Command, args []string) error {
		...
		output, err := controller.Execute(cmd.Context(), Request{Action: action, Args: args, Options: values})
		if err != nil {
			return err
		}
		if output != "" {
...
func NewWTS(controller Controller) *cobra.Command {
	...
	worktreePath := &cobra.Command{Use: "path <branch> [repository-id]", Args: rangeArgs("worktree path requires a branch and optional repository ID", 1, 2), RunE: run(controller, "worktree.path", nil)}
	var start string
	worktreeCreate := &cobra.Command{Use: "create <branch> [repository-id]", Args: rangeArgs("worktree create requires a branch and optional repository ID", 1, 2)}
	...
	worktreeRemove := &cobra.Command{Use: "remove <path-or-branch> [repository-id]", Aliases: []string{"rm"}, Args: rangeArgs("worktree remove requires a path or branch and optional repository ID", 1, 2)}
	...
	var rerun bool
	setup := &cobra.Command{Use: "setup <worktree> [repository-id]", Args: rangeArgs("worktree setup requires a path and optional repository ID", 1, 2)}
	...
	var relaunch bool
	launch := &cobra.Command{Use: "launch <worktree> [repository-id]", Args: rangeArgs("worktree launch requires a path and optional repository ID", 1, 2)}
	...
	attach := &cobra.Command{Use: "attach [repository-id]", Args: rangeArgs("attach accepts at most one repository ID", 0, 1), RunE: run(controller, "attach", nil)}
	var jsonOutput bool
	status := &cobra.Command{Use: "status [repository-id]", Args: rangeArgs("status accepts at most one repository ID", 0, 1)}
	...
	reconcile := &cobra.Command{Use: "reconcile [repository-id]", Args: rangeArgs("reconcile accepts at most one repository ID", 0, 1), RunE: run(controller, "reconcile", nil)}
	var pruneID, orphanID string
	cleanup := &cobra.Command{Use: "cleanup", Args: exactArgs("cleanup does not accept positional arguments", 0)}
	...
	for _, name := range []string{"install", "uninstall", "start", "stop", "status"} {
		daemonCmd.AddCommand(&cobra.Command{Use: name, Args: exactArgs("daemon "+name+" does not accept arguments", 0), RunE: run(controller, "daemon."+name, nil)})
	}
	root.AddCommand(configCmd, repoCmd, worktreeCmd, attach, status, reconcile, cleanup, daemonCmd)
	return root
}

func ExecuteWTS(ctx context.Context, controller Controller, stdout, stderr io.Writer, args []string) error {
