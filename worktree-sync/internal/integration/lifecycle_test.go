...
func TestExplicitLifecycleStatusBranchSafetyCleanupAndUnregister(t *testing.T) {
	...
	beforePreview := command(t, "", "tmux", "-L", socket, "list-windows", "-a", "-F", "#{session_id}|#{window_id}|#{window_name}|#{@wts-identity}")
	reconcilePreview, err := controller.Execute(context.Background(), forRepo("reconcile", nil, map[string]any{"dry_run": true}))
	require.NoError(t, err)
	require.Contains(t, reconcilePreview, "planned:")
	afterPreview := command(t, "", "tmux", "-L", socket, "list-windows", "-a", "-F", "#{session_id}|#{window_id}|#{window_name}|#{@wts-identity}")
	require.Equal(t, beforePreview, afterPreview)
	reconcileOutput, err := controller.Execute(context.Background(), forRepo("reconcile", nil, nil))
	require.NoError(t, err)
	require.Contains(t, reconcileOutput, "applied:")
