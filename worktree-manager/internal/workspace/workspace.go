...
// Paths are relative to the respective roots. Missing sources are skipped.
func (s *Service) copyFile(repoRoot, worktreeDir, relPath string) error {
	...
	if err != nil {
		return fmt.Errorf("open configured file %s: %w", relPath, err)
	}
	...
	if _, err := os.Stat(dst); err == nil {
		...
		return fmt.Errorf("stat copy destination %s: %w", relPath, err)
	}
	...
	if err := os.MkdirAll(filepath.Dir(dst), 0o755); err != nil { //nolint:gosec // 0755 is appropriate for worktree directories
		return fmt.Errorf("create directory for configured file %s: %w", relPath, err)
	}
	dstFile, err := os.Create(dst) //nolint:gosec // path is constructed from config, not user input
	if err != nil {
		return fmt.Errorf("create configured file %s: %w", relPath, err)
	}
	defer dstFile.Close() //nolint:errcheck // best-effort close; errors caught by io.Copy
	if _, err := io.Copy(dstFile, srcFile); err != nil {
		return fmt.Errorf("copy configured file %s: %w", relPath, err)
	}
	return nil
}
