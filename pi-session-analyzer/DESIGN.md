...
Every request requires the persisted auth token, mirroring `mcp-broker`: 32 random bytes hex-encoded (64 chars) at `${XDG_CONFIG_HOME:-~/.config}/pi-session-analyzer/auth-token` (file `0600`, parent directories `0750`), generated on first `dashboard` run and rotated with `token rotate` (restart to apply; the token value is never printed or logged — only the tokened startup URL is). The startup URL carries `?token=`; a valid query token is exchanged for an `HttpOnly`, `SameSite=Strict` session cookie and answered with a redirect that strips the token from the URL, so the secret leaves the address bar and history. `Authorization: Bearer` and the cookie authenticate everything else; all comparisons use `crypto/subtle.ConstantTimeCompare`. Unauthenticated API requests fail closed with 401; unauthenticated page loads land on the public `/unauthorized` page. The token is defense-in-depth — the loopback-only bind remains the load-bearing boundary.
...
Overview panels cover sessions, logged cost, call volume and classifiable per-tool errors, separate token categories, compactions, broker guards, current findings, detector gaps, goals, final stop reasons, record/turn distributions, and skill usage (SKILL.md reads grouped by skill target with invocation counts, session counts, and last-used recency, plus a per-bucket invocation trend). A sortable, keyset-paged session signal matrix bridges selected buckets to drilldown. The linear drilldown pages every record kind by `(source_line, kind_rank, id)`, lazily retrieves bounded collapsed text, anchors finding evidence exactly, and separately presents token sequence/compactions, tool coverage, source-ordered skill invocations with stream anchors, current findings, stale retained evidence, detector status, goal progression, and tolerant TODO progression/final state.
...
All assets and requests are same-origin. CSP, frame/referrer/type protections and `Cache-Control: no-store` apply to every response, including auth redirects. The `HttpOnly` auth session cookie is the sole cookie; there is no other browser persistence, and no external assets, telemetry, export/share/download/print controls, redaction claims, raw tier, arbitrary SQL, polling, or live tailing. Stored values are inserted only as text nodes. A persistent warning states that retained scrubbed content is private and not safe to share or screenshot.
...
V1 excludes export/share/download formats, present-safe redaction, live tailing or polling, remote/multi-user operation, TLS, telemetry, external assets, autonomous pruning, cost-product analytics, budgets/forecasts, statistical rankings, configuration fingerprints/cohorts, configuration proposals or edits, OpenInference/Phoenix export, arbitrary/custom dashboards, browser SQL, duration/latency waterfalls, branch/tree views, and any self-modifying reflection loop.
