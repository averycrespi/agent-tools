...
Overview panels cover sessions, logged cost, call volume and classifiable per-tool errors, separate token categories, compactions, broker guards, current findings, detector gaps, goals, final stop reasons, record/turn distributions, and skill usage (SKILL.md reads grouped by skill target with invocation counts, session counts, and last-used recency, plus a per-bucket invocation trend). A sortable, keyset-paged session signal matrix bridges selected buckets to drilldown. The linear drilldown pages every record kind by `(source_line, kind_rank, id)`, lazily retrieves bounded collapsed text, anchors finding evidence exactly, and separately presents token sequence/compactions, tool coverage, current findings, stale retained evidence, detector status, goal progression, and tolerant TODO progression/final state.
