...
var rules = []rule{
	...
	{"assignment", regexp.MustCompile(`(?i)(["']?\b(?:api[_-]?key|access[_-]?token|auth[_-]?token|client[_-]?secret|aws[_-]?secret[_-]?access[_-]?key|password|passwd|secret|token)\b["']?\s*[:=]\s*)(?:"[^"]*"|'[^']*'|[^\s,"'};]+)`), `${1}[REDACTED:assignment]`},
}
