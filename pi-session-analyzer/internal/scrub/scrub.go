...
var rules = []rule{
	...
	{"authorization", regexp.MustCompile(`(?i)(["']?\bauthorization\b["']?\s*[:=]\s*["']?[a-z][a-z0-9_-]*\s+)[A-Za-z0-9._~+/-]+={0,2}`), `${1}[REDACTED:authorization]`},
	{"assignment", regexp.MustCompile(`(?i)(["']?\b(?:[a-z][a-z0-9]*[_-])*(?:api[_-]?key|access[_-]?token|auth[_-]?token|client[_-]?secret|aws[_-]?secret[_-]?access[_-]?key|password|passwd|secret|token)\b["']?\s*[:=]\s*)(?:"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|[^\s,"'};]+)`), `${1}[REDACTED:assignment]`},
}
