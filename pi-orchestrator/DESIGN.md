...
Run-oriented MCP responses use explicit view structs rather than raw store structs. They expose persisted state, progress, cleanup fields, backing `pd` IDs, artifacts, and local path metadata already used by inspection surfaces, while omitting full workflow YAML snapshots, full inputs JSON, raw prompt text, environment values, and unbounded event/log content. Run listing is default-bounded and offset/limit-paginated. Supervisor and step logs are exposed only through offset/limit bounded log tools with size, next-offset, and truncation metadata.
