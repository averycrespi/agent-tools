...
const CookieName = "agent-mailbox-auth"
...
func MiddlewareWithLog(token string, next http.Handler, logf LogFunc) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if hasBearer(r, tokenBytes) || hasCookie(r, tokenBytes) {
			next.ServeHTTP(w, r)
			return
		}
		if strings.HasPrefix(r.URL.Path, "/dashboard/") || r.URL.Path == "/dashboard" {
			if qToken := r.URL.Query().Get("token"); qToken != "" && subtle.ConstantTimeCompare([]byte(qToken), tokenBytes) == 1 {
				...
				http.SetCookie(w, &http.Cookie{Name: CookieName, Value: token, Path: "/dashboard/", HttpOnly: true, SameSite: http.SameSiteStrictMode, MaxAge: int((365 * 24 * time.Hour) / time.Second)})
				clean := *r.URL
				q := clean.Query()
				q.Del("token")
				clean.RawQuery = q.Encode()
				//nolint:gosec // RequestURI is relative and same-origin.
				http.Redirect(w, r, clean.RequestURI(), http.StatusFound)
				return
			}
			logAuth(logf, "auth rejected path=%s", r.URL.Path)
