...
func (k *Keychain) Get(name string) (Record, error) {
	...
	if err := json.Unmarshal([]byte(raw), &env); err != nil {
		return Record{}, fmt.Errorf("%w: keychain item %q is not a valid egress-broker envelope; re-run `egress-broker credential set %s`",
...
func keychainRemediation() string {
	return "\n" +
		"  The OS keychain could not be reached. egress-broker does not fall back to a file:\n" +
