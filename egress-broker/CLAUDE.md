...
```bash
make build              # go build -o egress-broker ./cmd/egress-broker
...
```
cmd/egress-broker/     Cobra CLI; serve.go is the composition root
internal/
...
- **`EGRESS_BROKER_TEST_ALLOW_ADDRS` is test-only.** Every mock upstream listens
