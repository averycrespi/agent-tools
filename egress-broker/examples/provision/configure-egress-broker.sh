...
# egress-broker, and to trust the certificate it presents.
#
# Depends on the host running egress-broker (via `egress-broker serve` or the
# launchd agent; see docs/launchd.md). That process writes an auth token to
# $HOME/.config/egress-broker/auth-token and a CA certificate to
# $HOME/.local/share/egress-broker/ca.pem on the host.
#
...
TOKEN_FILE="$HOME/.config/egress-broker/auth-token"
CA_SOURCE="$HOME/.local/share/egress-broker/ca.pem"

# Where the guest trust store lives. Ubuntu is the sandbox-manager guest image.
CA_DEST="${EGRESS_BROKER_CA_DEST:-/usr/local/share/ca-certificates/egress-broker.crt}"
UPDATE_CA="${EGRESS_BROKER_UPDATE_CA:-update-ca-certificates}"

# The host, as seen from inside the Lima guest.
BROKER_HOST="${EGRESS_BROKER_HOST:-host.lima.internal}"
PROXY_PORT="${EGRESS_BROKER_PROXY_PORT:-8220}"
...
This file is created on the host by egress-broker and must be copied into the
...
See the egress-broker README for more information.
EOF
		exit 1
	fi
done
...
echo "Installing the egress-broker CA to $CA_DEST"
...
MARKER_START="# >>> egress-broker >>>"
MARKER_END="# <<< egress-broker <<<"
BASHRC="$HOME/.bashrc"
...
if grep -qF "$MARKER_START" "$BASHRC"; then
	echo "Replacing the existing egress-broker block in $BASHRC"
	sed -i "/^${MARKER_START}$/,/^${MARKER_END}$/d" "$BASHRC"
else
	echo "Adding the egress-broker block to $BASHRC"
fi

cat >>"$BASHRC" <<EOF
$MARKER_START
# Managed by configure-egress-broker.sh. Edits are lost on re-provision.
...
export EGRESS_BROKER_TOKEN="\$(cat "$TOKEN_FILE" 2>/dev/null)"

export HTTP_PROXY="http://x:\${EGRESS_BROKER_TOKEN}@${BROKER_HOST}:${PROXY_PORT}"
