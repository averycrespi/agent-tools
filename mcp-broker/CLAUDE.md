...
The embedded dashboard in `internal/dashboard/index.html` should stay visually aligned with the Pi Dispatcher dashboard. Preserve each dashboard's information architecture, but use the shared Agent Tools dashboard language:
...
- Keep `favicon.svg` in the same 64x64 rounded-square family as the Pi Dispatcher icon, changing only the glyph/accent for the product identity.
