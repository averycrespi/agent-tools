...
var ModelsOutputSchema = func() json.RawMessage {
	card := closed(object{"name": text(), "description": object{"type": "string"}, "release_date": object{"type": "string", "pattern": `^\d{4}-\d{2}-\d{2}([Tt]([01]\d|2[0-3]):[0-5]\d:[0-5]\d(\.\d+)?([Zz]|[+-]([01]\d|2[0-3]):[0-5]\d))?$`}}, "name", "description", "release_date")
	cards := array(card, 0)
	cards["maxItems"] = 256
