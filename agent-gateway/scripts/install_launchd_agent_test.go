...
func TestIntegrationLaunchdPlutilFixture(t *testing.T) {
	if os.Getenv("AGENT_GATEWAY_PLUTIL_FIXTURE") != "1" {
...
func newLaunchdFixture(t *testing.T) launchdFixture {
	...
	t.Setenv("AGENT_GATEWAY_TEST_ACCOUNT_PLIST", `<plist version="1.0"><dict><key>dsAttrTypeStandard:NFSHomeDirectory</key><array><string>`+escapedHome.String()+`</string></array></dict></plist>`)
	t.Setenv("AGENT_GATEWAY_TEST_GOPATH", gopath)
	t.Setenv("AGENT_GATEWAY_TEST_PLATFORM", "Darwin")
	t.Setenv("AGENT_GATEWAY_TEST_FORBIDDEN", filepath.Join(root, "forbidden-execution"))
	...
	writeTool("launchctl", `touch "$AGENT_GATEWAY_TEST_FORBIDDEN"; exit 1`)
	if runtime.GOOS == "darwin" {
		writeTool("plutil", `exec /usr/bin/plutil "$@"`)
	} else {
		executable, err := os.Executable()
		require.NoError(t, err)
		t.Setenv("AGENT_GATEWAY_TEST_EXECUTABLE", executable)
		writeTool("plutil", `AGENT_GATEWAY_PLUTIL_FIXTURE=1 GORACE="${GORACE:-} atexit_sleep_ms=0" exec "$AGENT_GATEWAY_TEST_EXECUTABLE" -test.run=^TestIntegrationLaunchdPlutilFixture$ -- "$@"`)
	}
	...
	t.Cleanup(func() {
		for _, path := range []string{os.Getenv("AGENT_GATEWAY_TEST_FORBIDDEN"), filepath.Join(home, ".local"), os.Getenv("HOME")} {
			_, err := os.Lstat(path)
	...
	for _, mode := range []string{"default", "xdg", "explicit"} {
		t.Run(mode, func(t *testing.T) {
			...
			case "explicit":
				...
				t.Setenv("AGENT_GATEWAY_TEST_GOPATH", "invalid-ignored")
	...
	for _, name := range []string{"relative-data", "relative-binary", "missing-value", "unknown-argument", "relative-xdg", "missing-binary", "multiple-gopath", "non-macos", "symlink-plist", "public-logs", "symlink-logs", "public-log-file", "invalid-template"} {
		t.Run(name, func(t *testing.T) {
			...
			case "multiple-gopath":
				t.Setenv("AGENT_GATEWAY_TEST_GOPATH", "/first:/second")
			...
			case "invalid-template":
				...
				require.NoError(t, os.WriteFile(filepath.Join(module, "examples", "launchd", "agent-gateway.plist"), []byte("not XML"), 0o600))
			}
			result := fixture.run(t, args...)
			require.NotZero(t, result.ExitCode)
			if expected != "" {
