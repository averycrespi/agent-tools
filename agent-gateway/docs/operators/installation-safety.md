...
Token copies have separate client compatibility and recovery purposes, not automatic removal eligibility. Provisioning still exports both canonical and legacy endpoint/token pairs from one authority; a retained old token copy must match the canonical file or provisioning fails closed. Follow the [client matrix](access-control.md#consumer-compatibility-qualification-and-rollback) rather than deleting legacy exports as installation cleanup.
