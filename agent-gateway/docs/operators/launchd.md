...
Prerequisites: the [Gateway build requirements](../../README.md#installation) and the macOS tools `dscl`, `plutil`, and `launchctl`. No Python or manual plist editing is needed. Run from this repository's `agent-gateway/` directory in one Terminal session as the intended logged-in user, without `sudo`.
