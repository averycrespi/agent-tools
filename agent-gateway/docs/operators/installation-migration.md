...
5. **Install, inspect, then explicitly start one service.** Install the intended production binaries using the separately authorized deployment procedure. The installer can preserve the archived literal custom argv, replacing only binary and data root:
   ...
   The installer never overwrites or starts. It rejects simultaneous listener/allowed-host overrides with `--from-plist`. After the six default argv elements, it accepts only separate-element `--allowed-host` (repeatable), `--log-level`, `--output` or `--json`. Duplicate singleton flags (including trailing data/listener flags), mixed output selections, equals spellings, unknown flags and positional/incomplete arguments require explicit reconciliation. Verify every selection, including nondefault flags, and the canonical logs/plist before an explicit bootstrap. Keep the archived old plist outside automatic-load paths. Follow [verification](launchd.md#verify), using the existing bearer and agent credentials, not newly issued substitutes. Expected native permission prompts need separate attended consent for the new binary; service IDs and generation handles do not change.
