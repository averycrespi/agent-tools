...
Operations capture desired and all credential revisions, use closed kinds/states/reasons, and permit only the declared scheduled/running transitions into immutable terminal state. Startup interruption is one marker-armed mutation. The repository retains the newest 64 terminal operations per server, never prunes nonterminal work, and advances a durable retained-floor watermark whenever pruning occurs. Server and operation lists capture insertion-sequence upper watermarks; later inserts are excluded and a cursor whose unconsumed position crossed the retained floor fails stale. Opt-in server table queries scan the bounded inventory under that watermark and synthesize each server's observed status from desired state, credentials, runtime, catalog, and reconciliation/dispatch/traversal saturation before global filtering and sorting. Each continuation binds the exact query, process-local handler epoch, and a digest of every observed identity/name/namespace/status/active-tool-count/desired-revision tuple. Changed recognition facts invalidate traversal explicitly rather than reusing a position in a reordered collection. These are administrative observations, not a globally simultaneous runtime snapshot or callability authority. Resources are assembled within the request only; no server/transport snapshot cache is retained. Opt-in operation history queries filter and order retained rows in SQL before bounded page selection. Their insertion watermark excludes later work; pruning generation and scheduled/running counts fence every legal mutable transition under that watermark (scheduled counts only decrease, and when unchanged, running counts only decrease). Terminal rows remain immutable. Changed snapshots fail stale rather than mixing pages; no migration or row cache is required. A separate at-most-two-row active projection with overflow indication reads only scheduled/running work and never traverses history to determine action eligibility. The [public contract](public-contract.md) owns exact query, ordering, and cursor vocabulary.
...
`internal/httpcredentials` owns reusable HTTP credential metadata separately from MCP server credentials. Schema 19 stores only permanent ID, bounded name, revision, one HTTPS host/port boundary with explicit wildcard opt-in, one header name and optional fixed prefix, timestamps, deletion tombstone, and opaque material-generation metadata. There are at most 256 live credentials and 1,024 permanent identities. No arbitrary templates or multiple-secret recipes exist. Host normalization and whole-grant containment delegate to `httppolicy`; wildcard spelling without opt-in, wildcard IPs, apex escape and port/scheme expansion fail closed.
...
The reconciliation manager owns a generation-fenced active publisher: every behavioral trigger synchronously fences and withdraws the old publication, retains exact process-local ownership, requires the injected driver to confirm stop before starting a replacement, conditionally publishes only the current candidate, and records operation success last. The trigger captures exact active/constructing candidates under the manager lock, then withdraws outside that lock before returning, even when reconciliation admission is saturated. This preserves publication's catalog-to-manager lock order and keeps the original handles available for verified stop. Unconfirmed stop retains only a cleanup handle, reports `stop_unconfirmed`, and gates replacement; if a concurrent verified stop has already removed both the driver handle and runtime ownership, that absence confirms cleanup instead of creating a permanent blocked stop.

OAuth-triggered replacement retains the exact queued or running operation independently of the replacement's operationless intent. Running work retains its original reconciliation attempt and cause until its worker and candidate cleanup return; queued displaced work performs only applicable prior-runtime cleanup, never activation. After verified cleanup, settlement atomically supersedes that exact nonterminal operation and records the original reconciliation outcome through the server repository's supplied-transaction audit seam. Already-terminal results, timestamps, attribution, and operation identity remain unchanged. Only acknowledged operation changes publish operation invalidations; a replacement's success cannot succeed the displaced operation.

Execution occupancy is released once by the returning work owner, not by an intermediate completion helper. For ordinary reconciliation completion only, the still-running owner may repeat terminal persistence after the typed storage admission-busy refusal, which proves no transaction or callback began. It makes at most four acquisition attempts with 25/50/100 ms waits, retaining the exact prepared audit outcome, candidate, and occupancy. Waiting releases the lifecycle lock; every acquisition revalidates work identity, generation, and drain under that lock, which remains held through admitted mutation cleanup. Drain cancels the wait. These bounds govern acquisition reattempts, never supply an active SQL deadline, and do not repeat startup, credential resolution, catalog discovery, or any external effect. Generic resource limits, storage latch, and uncertain writes are not retryable. Displaced settlement is not retried. Unconfirmed stop, exhausted acquisition attempts, or failed terminal persistence retains the exact settlement obligation and cleanup handle (when any) in memory, reports degraded status, and blocks replacement. Later callbacks coalesce but cannot replay failed work or discard the retained obligation. Beyond the bounded acquisition waits inside the original worker, there is no settlement queue, timer, watchdog, or age-based repair. Drain winning the lifecycle fence prevents supersession; startup conservatively interrupts remaining durable nonterminal operations rather than resuming them. Existing stopped-process recovery remains the recovery path for old orphan rows and retained settlement failures.
...
`internal/downstream` owns monotonic runtime-local request IDs and strict closed JSON-RPC envelopes before protocol-era projection. Its stdio connection writes bounded NDJSON only through the supervised runtime and requires verified runtime stop on closure. Its Streamable HTTP connection emits validated per-server non-secret custom headers plus role-built JSON Content-Type, JSON/SSE Accept, protocol, method, optional name, validated parameter mirrors, runtime-bound legacy session, and server-scoped Authorization; bounded JSON or one bounded SSE event is returned without inheriting inbound authority, IDs, sessions, progress/task fields, headers, or metadata. Runtime-root closure cancels active HTTP exchanges and waits for each exchange to return before confirming transport stop; expiry of the stop context remains unconfirmed.

Optional Streamable HTTP `headers` is a string map of ordinary plaintext configuration, not a secret sink. It survives configuration readback, SQLite restart, and backups. Existing definitions remain valid; omission, null, or an empty map in a complete replacement clears custom headers, while omitting PATCH transport preserves them. Header changes are behavioral desired revisions: existing concurrency controls fence the old runtime and catalog, stop it before replacement, and publish only a freshly discovered current catalog. Each transport owns a private copy, including auto-fallback replacements. All MCP requests use it; OAuth metadata, registration, token and revocation roles, other servers, and browser response headers never do. Inbound request headers are not a source.

The executable policy in `internal/contract/upstream_headers.go` admits at most 16 token names of 1–128 ASCII bytes, with case-insensitive uniqueness, 4096 bytes per value and 8192 total name/value bytes. Values are empty or printable ASCII without surrounding whitespace, so accepted bytes are never trimmed or normalized. Reserved prefixes are `mcp-`, `proxy-`, `sec-`, `x-forwarded-`, `content-`, `accept-`, and `if-`. Reserved exact names are `authorization`, `authentication-info`, `www-authenticate`, `cookie`, `cookie2`, `set-cookie`, `set-cookie2`, `api-key`, `x-api-key`, `x-auth-token`, `x-access-token`, `x-authorization`, `host`, `connection`, `keep-alive`, `te`, `trailer`, `transfer-encoding`, `upgrade`, `expect`, `forwarded`, `via`, `x-real-ip`, `x-original-url`, `x-rewrite-url`, `x-http-method-override`, `origin`, `referer`, `accept`, `user-agent`, `range`, `cache-control`, `pragma`, `max-forwards`, and `date`. Matching ignores case. Validation rejects collisions rather than allowing ordering to overwrite protected fields. The operator guide owns the GitHub `X-MCP-Toolsets` example and configuration procedure.

The remote boundary validates the combined outgoing name/value lengths and field count before handoff, including HTTP/1-generated Host, Connection, Content-Length and default User-Agent where applicable, under the existing 100-field/32768-byte/8192-value-byte bounds. Suppressed User-Agent may conservatively occupy one field. This includes runtime Authorization, session, MCP metadata and parameter mirrors; custom-header admission alone cannot promise that an arbitrarily large later parameter set or credential will fit.
...
A process-local registry serializes every capacity reservation, durable commit, immutable active replacement, stale retention, and withdrawal under one global lock; active generation cursors and per-server status change atomically, while a post-commit stale runtime publishes nothing. Only exactly current active generations are discoverable and callable. Opt-in aggregate table queries select compact descriptor references under this same registry lock, apply tool/server/status filters and deterministic sorting across the collection, and clone only the selected at-most-50 full descriptors. Their query-bound position cursors also pin the active generation and insertion upper watermark; publication, stale retention, or withdrawal invalidates them. Legacy insertion-order and query cursors are not interchangeable.
