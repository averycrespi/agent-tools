...
| ---------------------------------------------------- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------- | ------ | ----------- | ---------------- | ------------- |
| `GET /api/v1/servers`                                | `ServerListQuery\|ServerTableQuery`                                                       | `Page<Server>` / 200                                      | yes    | no          | no               | no            |
...
| `GET /api/v1/servers/{id}/operations`                | `ServerOperationListQuery`, `ServerOperationTableQuery`, or `ActiveServerOperationsQuery` | legacy page, query page, or active projection / 200       | yes    | no          | no               | no            |
...
| `GET /api/v1/catalog`                                | `CatalogListQuery\|CatalogTableQuery`                                                     | `CatalogPage` / 200                                       | yes    | no          | no               | no            |
| `GET /api/v1/servers/{id}/descriptors`               | `DescriptorListQuery\|DescriptorTableQuery`                                               | `Page<ToolDescriptor>\|Page<ToolDescriptorSummary>` / 200 | yes    | no          | no               | no            |
...
The executable collection request schemas are `ServerListQuery|ServerTableQuery`, `DescriptorListQuery|DescriptorTableQuery`, and `CatalogListQuery|CatalogTableQuery`. The executable descriptor-collection success schema is `Page<ToolDescriptor>|Page<ToolDescriptorSummary>`; the selected representation determines which closed item shape is returned. In the executable mechanics tables, `None` means no query or request body and `Empty` means an empty response body. A page is exactly `{items,next_cursor}`. List query schemas permit only their declared query members. Principal lists additionally support bounded `name`, `state`, `visibility`, `sort`, and `direction`; grant lists support `principal_id`, `server_id`, `identity`, `principal`, `target`, `effect`, `state`, `sort`, `direction`, and optional `representation=table`. The executable principal-collection schema is `Page<Principal>|QueryPage<Principal>` and the grant-collection schema is `Page<Grant>|QueryPage<Grant>|QueryPage<GrantTableItem>`. `QueryPage<T>` is exactly `{items,next_cursor,total_count,offset}`: the count is the exact number of matching records before slicing and the offset is the zero-based first-row position, both nonnegative JSON integers from the same snapshot as the items. Empty results have zero count and offset. Any new query option selects `QueryPage`; `representation=table` selects items exactly `{grant,principal_display_name,server_display_name}`. Omitting the new query options retains `Page<Principal>` or `Page<Grant>`, including grants filtered only by `principal_id`/`server_id`. Both preserve their default ordering and page sizes when query options are omitted. See [Identity and Authorization](identity-and-authorization.md#principal-and-grant-contract) for exact matching, ordering, and process-bound five-minute cursor semantics. Legacy descriptor lists permit `retired` plus the sole optional alternate representation `representation=summary`. Supplying any of `tool`, `status`, `sort`, or `direction` selects `DescriptorTableQuery`, which accepts only those fields plus `cursor` and canonical decimal `limit` from 1 through 50 (default 50). It returns the unchanged full-descriptor `{items,next_cursor}` page without totals. `tool` is a substring search over the external name after NFKC normalization and lowercase conversion. Both its supplied and normalized forms are bounded to 256 UTF-8 bytes without control/format characters. `status` is `available` (not retired) or `retired`; `sort` is `tool`, `status`, or `last-seen`; `direction` is `ascending` or `descending` and requires `sort`. With no sort, table order is last-seen descending; an explicit sort defaults to ascending. All sorts use immutable ID-ascending ties. Filters apply conjunctively before page selection. Unknown, repeated, empty, invalid, or conflicting legacy options fail. Cursors bind the exact query, server identity, catalog revision, and insertion watermark; a mismatch returns `409 stale_cursor` and requires a fresh first page. Available durable evidence never establishes callability.

Server table queries opt in with `name`, `namespace`, `status`, `sort`, or `direction`. `name` searches display name or ID; `namespace` searches namespace. Text uses the same bounded NFKC/lowercase substring rules as descriptor queries. Status is exactly one of `Ready`, `Connecting`, `Authorization required`, `Authentication unavailable`, `Capacity saturated`, `Disabled`, `Deleted`, or `Needs attention`, matching the browser's precedence over desired/runtime/credential/catalog/capacity observations. Sort is `name`, `id`, `namespace`, `status`, or `tools` (active count); defaults are name ascending. Catalog table queries opt in with `tool`, `server`, `status`, `sort`, or `direction`: text searches external tool name and server display name, status is `available` or `issue`, sort is `tool` or `server`, and defaults are tool ascending. Both support `ascending`/`descending` directions requiring an explicit sort, canonical decimal limits 1–50 (default 50), and cursors; unknown/repeated/empty/invalid options fail. Text ordering is NFKC/lowercase lexical with ID-ascending ties. Filters are conjunctive and apply before page selection. Legacy clients omitting new options retain their order, page limits, and closed representations; table-query responses also retain those representations and add no totals.

Server query cursors bind the exact query, insertion upper watermark, process-local handler epoch, and every observed identity/name/namespace/status/active-count/desired-revision tuple. Changed recognition facts return `409 stale_cursor` instead of silently reordering traversal; later server insertions remain excluded by the watermark. Catalog query cursors bind the exact query, active generation, and insertion upper watermark under the active-registry lock; publication, staleness, or withdrawal invalidates traversal. Query cursors are not interchangeable with legacy insertion-order cursors. Clients must discard stale history and start a fresh first page; no representation asserts callability or permission.
