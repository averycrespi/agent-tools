...
Schema 20 adds one revisioned HTTP default per principal, backfilled to block and seeded to block by the insert trigger. Principal creation accepts optional `http_default` (`allow` or `block`); omission preserves block, while explicit null or other values fail validation. An explicit allow is set within the same creation transaction, retaining initial principal/default revision 1. The canonical Principal exposes `http_default`; principal creation and GET/PATCH are its sole public writer/reader contracts. The dedicated HTTP-default resource is removed. Existing stored allow/block values, the insert trigger, backup lineage and internal default revisions are preserved without a schema migration. HTTP defaults never supply credential, tunnel or private-network permission; MCP visibility/default_grant and singular credential-slot semantics remain unchanged.
