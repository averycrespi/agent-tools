...
The common real-binary runner requires a positive timeout and per-stream byte cap, captures stdout and stderr separately, reports truncation and exit status, owns an identity-revalidated process group, applies bounded TERM/KILL/reap cleanup when its context expires, and can signal a bounded started process for lifecycle tests. Successful `Setpgid` at startup establishes the fresh group's ID from its unreaped child, including fast-exiting Darwin children; this does not replace identity revalidation during later cleanup. The single E2E harness and acceptance executor inherit that ownership through an outer cleanup ledger and fail on surviving processes, listeners, or temporary roots. Nested suite executors clean their owned command groups but leave the inherited ledger to the outer acceptance owner; cleaning that ledger inside a leaf would terminate its still-live parent. Component-specific fault hooks, protocol fixtures, and barriers remain with their owning packages. The runtime hard-crash fixture requires an actual readiness frame and live child before publishing its private PID record; stdout from a failed test or a closed frame channel is not readiness. Its owner runs under the bounded binary runner, retains bounded exit diagnostics, and must die by the deliberate SIGKILL before the orphan-survival and fresh-supervisor assertions. The fixture deadlines and temporary PID record do not add production process adoption or restart authority.

The upstream-header saturation fixture completes its enable mutations before triggering reconciliation, then waits for each blocker's discovery entry before triggering the next. Earlier discoveries remain blocked until all four slots are occupied. Freeing one slot admits the replacement; the other blockers stay parked until composition drain cancels them, avoiding unrelated catalog/operation writes during replacement. This avoids incidental storage contention without serializing the saturation scenario, weakening fencing/catalog assertions, or extending its five-second waits.
...
| `go install github.com/averycrespi/agent-tools/mcp-gateway/cmd/mcp-gateway@latest` | `go install github.com/averycrespi/agent-tools/agent-gateway/cmd/agent-gateway@latest` ; checkout `make -C agent-gateway install` also publishes only the canonical name |
...
This is a purpose-based inventory, not a zero-string target. The installation safety guide records the rollout-owner attestation; source tests do not independently qualify host adoption. The [installation selection and cleanup guidance](../operators/installation-safety.md#retired-executable-and-operator-cleanup) and [client matrix](../operators/access-control.md#consumer-compatibility-qualification-and-rollback) own operator selections.

| Residual names and owners                                                                                                               | Retained purpose                                                                                                                                   |
| --------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MCP_GATEWAY_ENDPOINT`, `MCP_GATEWAY_AGENT_TOKEN` in the canonical provision script/tests and client docs                               | Explicitly retained consumer compatibility; both pairs derive from one canonical current token file, never independent endpoint/bearer authorities |
| `~/.config/mcp-gateway/agent-token`, old managed markers in provisioning/tests                                                          | Conflict/unsafe-input detection and convergent marker replacement only; no old-file selection or old script alias                                  |
| Legacy roots and completed relocation records in `internal/paths` and their fixtures                                                    | Safety refusal and exact completed-tombstone recognition; custom selections remain supported, while migration and host-inspection code are retired |
| `mcp-gateway` in renamed-binary E2E fixtures and the root `.gitignore`                                                                  | Regression coverage and retained old local output; Make never publishes that name or removes existing bytes                                        |
| MCP advertised `mcp-gateway` / `mcp-gateway/s2`, reserved `mcp_gateway.*`, validation URNs                                              | Immutable protocol/schema/policy identities in ingress, downstream, self-service, catalog and historical SQL                                       |
| `mcp-gateway` credential/fingerprint/cursor domains, keyring prefixes, `mgw_*` credentials/IDs, `MGW1` database identity                | Security/format and durable lineage, not executable branding; retain tests, backups and recovery fixtures                                          |
| `mcp_gateway_theme`, `mcp_gateway_session` in browser/session owners and generated assets                                               | Existing preference migration and expiry-only old-cookie safety; no old-cookie authentication                                                      |
| Private retired `MCP_GATEWAY_*` controls in test/development guards                                                                     | Actionable stale-input rejection, not supported configuration aliases                                                                              |
| External `agent-config` extension/settings paths, old-to-new doc matrices/anchors, CI deleted/renamed-path fixtures, historical reports | External consumer identity or historical/migration evidence; not current executable/script guidance                                                |
| `mcp-broker` and broker-owned names                                                                                                     | Separate tool, outside Gateway retirement                                                                                                          |
