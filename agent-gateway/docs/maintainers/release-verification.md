...
This is a purpose-based inventory, not a zero-string target. The rollout owner authorized retirement on their supported-scope attestation without independently enumerated host evidence; preserve that qualification boundary separately from passing source tests. The [supported installation matrix and cleanup procedure](../operators/installation-migration.md#retired-executable-and-operator-cleanup) and [client matrix](../operators/access-control.md#consumer-compatibility-qualification-and-rollback) own operator selections.
