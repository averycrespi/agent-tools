...
Integration retains five minutes per package and allows twelve minutes for compilation and the complete package group (the exact-base macOS CI command took 648 seconds). The native wrapper's seven-minute budget includes its count-one material prerequisite and setup; the native Go package still has a ten-second timeout. This changes no native-access consent boundary. The fifteen-minute overall release profile remains a separate ceiling, not a promise that cold release acceptance fits it; ordinary CI does not qualify that profile.
...
| --------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MCP_GATEWAY_ENDPOINT`, `MCP_GATEWAY_AGENT_TOKEN` in client configuration guidance                                                      | Explicitly retained consumer compatibility; both pairs derive from one canonical current token file, never independent endpoint/bearer authorities |
| `~/.config/mcp-gateway/agent-token`, old managed markers in historical client profiles                                                  | Operator-owned conflict reconciliation only; script-enforced detection and marker replacement are retired                                          |
