...
func (store *Store) verify(ctx context.Context) error {
	...
	if err := store.verifyMigrationStructure(ctx, "018_traffic_selection.sql"); err != nil {
		return fmt.Errorf("%w: %w", ErrInvalidDatabase, err)
	}
	if err := store.verifyMigrationStructure(ctx, "019_http_credentials.sql"); err != nil {
		return fmt.Errorf("%w: %w", ErrInvalidDatabase, err)
	}
	if err := store.verifyMigrationStructure(ctx, "020_http_grants.sql"); err != nil {
		return fmt.Errorf("%w: %w", ErrInvalidDatabase, err)
	}
	if err := store.verifyMigrationStructure(ctx, "021_http_ca.sql"); err != nil {
		return fmt.Errorf("%w: %w", ErrInvalidDatabase, err)
	}
