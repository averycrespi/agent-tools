...
func TestPreCutoverAgentCredentialRemainsUsable(t *testing.T) {
	repository, store := newRepository(t, nil)
	principal := mustCreatePrincipal(t, repository)
	created, err := repository.IssueCredential(t.Context(), principal.ID, "1")
	require.NoError(t, err)
	// Frozen pre-cutover verifier/fingerprint; do not derive with current helpers.
	const bearer = "mgw_agent_WlpaWlpaWlpaWlpaWlpaWlpaWlpaWlpaWlpaWlpaWlo"
	const fingerprint = "24d07f1b75516a60"
	require.NoError(t, store.Mutate(t.Context(), func(tx *sql.Tx) error {
		_, updateErr := tx.ExecContext(t.Context(), `UPDATE principals SET credential_verifier = X'811ab880faa5ba9c2c79b08827f03d450af073e2dbb1b54164a21fe90a98017f', credential_fingerprint = ? WHERE id = ?`, fingerprint, principal.ID)
		return updateErr
	}))
	...
	assert.Equal(t, created.Principal.Credential.ID, lease.Binding().CredentialID)
	assert.Equal(t, fingerprint, lease.Binding().CredentialFingerprint)
	candidate := agentVerifierForBearer(bearer)
	assert.Equal(t, "811ab880faa5ba9c2c79b08827f03d450af073e2dbb1b54164a21fe90a98017f", hex.EncodeToString(candidate[:]))
}
...
func TestAuthenticationSourceUsesConstantTimeBoundedScanAndOneProductionBundle(t *testing.T) {
	...
	rootSource, err := os.ReadFile(filepath.Join("..", "..", "cmd", "agent-gateway", "root.go"))
	require.NoError(t, err)
