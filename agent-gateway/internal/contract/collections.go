...
var collectionContracts = []CollectionContract{
	...
	{Pattern: "/api/v2/mcp/catalog", DefaultOrder: "tool ascending", DefaultLimit: 50, MaximumLimit: 50, SuccessSchema: "CatalogPage", QueryMembers: []string{"cursor", "limit", "tool", "server", "status", "sort", "direction"}},
	{Pattern: "/api/v2/mcp/servers/{id}/descriptors", DefaultOrder: "last-seen descending", DefaultLimit: 50, MaximumLimit: 50, SuccessSchema: "Page<ToolDescriptor>", QueryMembers: []string{"cursor", "limit", "tool", "status", "sort", "direction", "projection"}, Projections: []string{"full", "summary"}},
	{Pattern: "/api/v2/mcp/servers/{id}/operations", DefaultOrder: "created descending", DefaultLimit: 50, MaximumLimit: 50, SuccessSchema: "QueryPage<ServerOperation>", QueryMembers: []string{"cursor", "limit", "action", "status", "sort", "direction", "projection"}, Projections: []string{"active"}},
	...
	{Pattern: "/api/v2/grants", DefaultOrder: "description ascending", DefaultLimit: 50, MaximumLimit: 100, SuccessSchema: "QueryPage<GrantTableItem>", QueryMembers: []string{"cursor", "limit", "principal_id", "server_id", "identity", "principal", "target", "effect", "state", "sort", "direction"}},
	{Pattern: "/api/v2/grant-requests", DefaultOrder: "submitted descending", DefaultLimit: 50, MaximumLimit: 100, SuccessSchema: "QueryPage<GrantRequestTableItem>", QueryMembers: []string{"cursor", "limit", "principal_id", "state", "request", "principal", "target", "scope", "sort", "direction"}},
	{Pattern: "/api/v2/invocations", DefaultOrder: "insertion_sequence descending", DefaultLimit: 50, MaximumLimit: 100, SuccessSchema: "InvocationPage", QueryMembers: []string{"cursor", "limit", "principal_id", "server_id", "requested_name", "admission_class", "decision", "outcome", "tool", "principal", "search_locale"}},
