...
var controlPlaneCapabilities = []ControlPlaneCapability{
	...
	{ID: "http-default", Operation: "HTTP principal default", WebControl: "Principal / Edit principal", CLIUses: []string{"http default get ID", "http default update ID --file PATH [--etag ETAG]"}, Mechanics: "canonical principal GET/PATCH; unified principal ETag; default block"},
	...
	{ID: "status", Operation: "Detailed status", WebControl: "Overview/System", CLIUses: []string{"doctor --online"}, Mechanics: "read-only checklist and authenticated GET status"},
	...
	{ID: "principal-read", Operation: "Principal list/get", WebControl: "Access / Principals", CLIUses: []string{"agent list", "agent get ID"}, Mechanics: "cursor/limit; bodyless"},
	{ID: "principal-create", Operation: "Principal create", WebControl: "Access / Principals / Create", CLIUses: []string{"agent create --display-name NAME --visibility VISIBILITY"}, Mechanics: "required direct display name and visibility"},
	{ID: "principal-update", Operation: "Principal update", WebControl: "principal detail / Edit", CLIUses: []string{"agent update ID [--etag ETAG] [--display-name NAME] [--visibility VISIBILITY] [--state STATE] [--http-default POLICY]"}, Mechanics: "nonempty direct patch; automatic or explicit ETag; confirmation"},
	{ID: "principal-credential-issue", Operation: "Agent credential issue/rotate", WebControl: "principal detail / Credential", CLIUses: []string{"agent credential issue ID [--etag ETAG] [--secret-output NEW_PATH]", "agent credential rotate ID [--etag ETAG] [--secret-output NEW_PATH]"}, Mechanics: "empty/occupied slot preflight; atomic rotation; one-time sink; no replay"},
	{ID: "principal-credential-revoke", Operation: "Agent credential revoke", WebControl: "principal detail / Credential", CLIUses: []string{"agent credential revoke ID [--etag ETAG]"}, Mechanics: "exact {}; automatic or explicit ETag; confirmation"},
...
func ControlPlaneLifecycleManifest() []ControlPlaneCapability {
	return []ControlPlaneCapability{
		...
		{ID: "cli-initialize", Operation: "Stopped initialize", CLIUses: []string{"init"}, Mechanics: "offline compatibility", CLIScenario: "cli.initialize"},
		{ID: "cli-admin-reset", Operation: "Stopped admin reset", CLIUses: []string{"maintenance reset-admin-credentials"}, Mechanics: "stopped all-authority recovery", CLIScenario: "cli.admin-reset"},
		{ID: "cli-storage-verify", Operation: "Stopped current storage verification", CLIUses: []string{"maintenance verify-and-recover-storage"}, Mechanics: "exclusive ownership; recognized recovery only; no replacement", CLIScenario: "cli.storage-verify"},
		{ID: "cli-backup-restore", Operation: "Stopped backup restore", CLIUses: []string{"maintenance restore-backup BACKUP_ID"}, Mechanics: "exclusive ownership; verified lineage; one-time replacement bearer; no replay", CLIScenario: "cli.backup-restore"},
	}
}
