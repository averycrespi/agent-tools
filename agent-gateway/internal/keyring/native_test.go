...
func TestNativeDisposableKeyringRoundTrip(t *testing.T) {
	if _, exists := os.LookupEnv("MCP_GATEWAY_KEYRING_NATIVE"); exists {
		t.Fatal("MCP_GATEWAY_KEYRING_NATIVE is retired; use AGENT_GATEWAY_KEYRING_NATIVE")
	}
	if os.Getenv("AGENT_GATEWAY_KEYRING_NATIVE") != "1" {
