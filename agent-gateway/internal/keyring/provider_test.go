...
func TestSystemAdapterConformanceUsesGoKeyringInIsolatedProcess(t *testing.T) {
	if os.Getenv("AGENT_GATEWAY_KEYRING_CONFORMANCE_HELPER") == "1" {
		oskeyring.MockInit()
		adapter := systemAdapter{}
		require.NoError(t, adapter.Set("agent-gateway-conformance", "item", "value"))
		value, err := adapter.Get("agent-gateway-conformance", "item")
		require.NoError(t, err)
		assert.Equal(t, "value", value)
		require.NoError(t, adapter.Delete("agent-gateway-conformance", "item"))
		_, err = adapter.Get("agent-gateway-conformance", "item")
	...
	command.Env = append(os.Environ(), "AGENT_GATEWAY_KEYRING_CONFORMANCE_HELPER=1")
