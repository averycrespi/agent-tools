...
func testProductionSourceOwnershipGuards(t *testing.T, root string, sources []productionSource) {
	...
	for _, source := range sources {
		...
		if source.path != "cmd/agent-gateway/root.go" && source.path != "internal/mcpingress/handler.go" && strings.Contains(source.contents, "mcpingress.Options{") {
	...
	assert.Contains(t, e2eHome, `AGENT_GATEWAY_E2E_ACCOUNT_HOME`, "internal/paths/home_e2e.go: test seam must use its closed environment name")
...
func TestRootUsesOneAtomicCompositionForControlAndAgentIngress(t *testing.T) {
	...
	for _, required := range []string{
		...
		assert.Contains(t, rootSource, required, "cmd/agent-gateway/root.go: missing production symbol %s", required)
	}
...
func TestDrainRootKeepsEventsAndStorageUntilCompositionSettles(t *testing.T) {
	...
	assert.True(t, joined < unconfirmed && unconfirmed < storageClosed,
		"cmd/agent-gateway/root.go: an unconfirmed drain must return before explicit storage closure")
...
func TestSourceNegativeFixturesReportExactPathAndSymbol(t *testing.T) {
	fixtures := []struct {
		...
		{
			name: "ordinary root deny all", path: "cmd/agent-gateway/root.go",
			contents: `package main
import "github.com/averycrespi/agent-tools/agent-gateway/internal/mcpingress"
var _ = mcpingress.DenyAllAuthenticator{}
`,
			want: "cmd/agent-gateway/root.go: prohibited production ingress symbol mcpingress.DenyAllAuthenticator{}",
		},
		{
...
`,
		...
		{
			name: "direct root invocation import", path: "cmd/agent-gateway/root.go",
			contents: `package main
import "github.com/averycrespi/agent-tools/agent-gateway/internal/invocation"
var _ invocation.Service
`,
			want: "cmd/agent-gateway/root.go: prohibited invocation import github.com/averycrespi/agent-tools/agent-gateway/internal/invocation",
		},
		{
...
`,
			want: "internal/discovery/bad.go: prohibited discovery import github.com/averycrespi/agent-tools/agent-gateway/internal/keyring",
		},
		{
...
func productionSliceViolations(source productionSource) []string {
	...
	if source.path == "cmd/agent-gateway/root.go" {
...
func s3MigrationViolations(t *testing.T, root string) []string {
	...
	for _, path := range paths {
		contents, readErr := os.ReadFile(path)
		require.NoError(t, readErr)
		if filepath.Base(path) == "008_authorization.sql" || filepath.Base(path) == "012_grant_names.sql" || filepath.Base(path) == "013_grant_descriptions.sql" || filepath.Base(path) == "016_read_only_grants.sql" || !s3SQLVerb.Match(contents) {
			continue
		}
		relative, relErr := filepath.Rel(root, path)
		require.NoError(t, relErr)
