			...
			for _, field := range structure.Fields.List {
				for _, name := range field.Names {
					...
					if name.Name == fieldName {
						...
						assert.Equal(t, "github.com/averycrespi/agent-tools/agent-gateway/internal/accesstarget", source.selectorPackage(selector), key)
						found = true
					}
				}
			}
...
func TestAccessTargetOwnershipNegativeFixtures(t *testing.T) {
	for _, fixture := range []struct{ path, contents string }{
		...
		{"internal/authorization/bad.go", "package authorization\nimport \"github.com/averycrespi/agent-tools/agent-gateway/internal/catalog\"\nvar _ = catalog.SyntheticSnapshot\n"},
		{"internal/contract/bad.go", "package contract\nimport \"github.com/averycrespi/agent-tools/agent-gateway/internal/accesstarget\"\nvar _ accesstarget.MCP\n"},
	} {
