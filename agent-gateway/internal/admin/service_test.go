...
func TestPreCutoverAdministratorCredentialRemainsUsable(t *testing.T) {
	store, _ := newStore(t)
	service := NewService(store, testutil.NewFakeClock(testNow), testutil.NewFakeEntropy(bytes.Repeat([]byte{0x5a}, 42)))
	initial, err := service.Initialize(t.Context(), new(memorySink))
	require.NoError(t, err)
	// Frozen pre-cutover bytes, not derived through the implementation under test.
	const bearer = "mgw_admin_WlpaWlpaWlpaWlpaWlpaWlpaWlpaWlpaWlpaWlpaWlo"
	const fingerprint = "17a7ccfe1bbfdce7"
	require.NoError(t, store.Mutate(t.Context(), func(tx *sql.Tx) error {
		_, updateErr := tx.ExecContext(t.Context(), `UPDATE admin_credentials SET verifier = X'd38823616c1a3751564f41f9a041ab8d9beb7de0bb0fb34e31dbafd265870e88', fingerprint = ? WHERE id = ?`, fingerprint, initial.ID)
		return updateErr
	}))
	authenticated, err := service.Authenticate(t.Context(), bearer)
	require.NoError(t, err)
	assert.Equal(t, initial.ID, authenticated.ID)
	assert.Equal(t, fingerprint, authenticated.Fingerprint)
	assert.Equal(t, fingerprint, initial.Fingerprint, "new issuance must preserve the same fingerprint derivation")
}
