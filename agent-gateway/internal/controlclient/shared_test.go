...
func TestControlClientSharedContract(t *testing.T) {
	...
	t.Run("one-page query and request metadata", func(t *testing.T) {
		path, err := BuildListPath("/api/v2/mcp/invocations", ListOptions{
		...
		require.NoError(t, err)
		assert.Equal(t, "/api/v2/mcp/invocations?cursor=cursor%2Fvalue&decision=ALLOW&limit=25&principal_id=01ARZ3NDEKTSV4RRFFQ69G5FAV", path)
		_, err = BuildListPath("/api/v2/mcp/invocations", ListOptions{Limit: 101})
		assert.ErrorIs(t, err, ErrInvalidInput)
		_, err = BuildListPath("/api/v2/mcp/invocations", ListOptions{Filters: map[string]string{"invented": "value"}, AllowedFilters: []string{"decision"}})
		assert.ErrorIs(t, err, ErrInvalidInput)
