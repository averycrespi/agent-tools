...
func TestUpstreamHeadersAPIConfigurationAndIsolation(t *testing.T) {
	...
	created := perform(handler, http.MethodPost, "/api/v2/mcp/servers", `{"namespace":"headers","display_name":"Headers","enabled":false,"transport":`+transport+`}`, headers)
	...
	got := perform(handler, http.MethodGet, "/api/v2/mcp/servers/"+testID, "", headers)
	...
	patched := perform(handler, http.MethodPatch, "/api/v2/mcp/servers/"+testID, `{"transport":`+transport+`}`, headers)
	...
	for _, raw := range []string{`{"Authorization":"secret-canary"}`, `{"X-Test":null}`, `{"X-Test":"one","x-test":"two"}`} {
		response := perform(handler, http.MethodPatch, "/api/v2/mcp/servers/"+testID, `{"transport":{"kind":"streamable_http","url":"https://resource.example/mcp","protocol_mode":"auto","authentication":{"mode":"none"},"headers":`+raw+`}}`, headers)
