...
func TestGrantRequestListItemApproveRejectAndPrivacy(t *testing.T) {
	...
	listed := perform(handler, http.MethodGet, "/api/v2/mcp/grant-requests?limit=1&principal_id="+testID+"&state=pending", "", headers)
	...
	continued := perform(handler, http.MethodGet, "/api/v2/mcp/grant-requests?limit=2&principal_id="+testID+"&state=pending&cursor="+*page.NextCursor, "", headers)
	...
	item := perform(handler, http.MethodGet, "/api/v2/mcp/grant-requests/"+service.item.ID, "", headers)
	...
	approved := perform(handler, http.MethodPost, "/api/v2/mcp/grant-requests/"+service.item.ID+"/approve", `{"description":"Approved access","approved_policy":{"scope":"server","target":"sample","constraint":null,"duration_seconds":null,"future_tools_acknowledged":true}}`, approveHeaders)
	...
	rejected := perform(handler, http.MethodPost, "/api/v2/mcp/grant-requests/"+service.item.ID+"/reject", `{"reason":"policy_conflict"}`, rejectHeaders)
...
func TestGrantRequestTableQueryWireContract(t *testing.T) {
	...
	handler := newGrantRequestHandler(t, service)
	headers := map[string]string{"Authorization": "Bearer " + testBearer}
	response := perform(handler, http.MethodGet, "/api/v2/mcp/grant-requests?request="+service.item.ID+"&principal=agent&target=Sample&scope=tool&state=pending&sort=submitted&direction=ascending", "", headers)
	...
	for _, query := range []string{"representation=table", "representation=detail", "direction=descending", "scope=other", "sort=unknown", "principal=%00", "target=a&target=b"} {
		rejected := perform(handler, http.MethodGet, "/api/v2/mcp/grant-requests?"+query, "", headers)
...
func TestGrantRequestStrictQueriesBodiesPreconditionsAndProblems(t *testing.T) {
	...
	for _, path := range []string{
		"/api/v2/mcp/grant-requests?unknown=x", "/api/v2/mcp/grant-requests?limit=1&limit=2",
		"/api/v2/mcp/grant-requests?principal_id=", "/api/v2/mcp/grant-requests?state=unknown", "/api/v2/mcp/grant-requests?cursor=%ZZ",
	} {
	...
	badCursor := perform(handler, http.MethodGet, "/api/v2/mcp/grant-requests?cursor=abc", "", headers)
	...
	missing := perform(handler, http.MethodPost, "/api/v2/mcp/grant-requests/"+service.item.ID+"/approve", body, contentHeaders)
	...
	for _, etag := range []string{"*", `W/` + contract.GrantRequestETag(service.item.ID, "1"), contract.GrantRequestETag(testServerID, "1"), `"grant-request-` + service.item.ID + `-01"`} {
		...
		response := perform(handler, http.MethodPost, "/api/v2/mcp/grant-requests/"+service.item.ID+"/approve", body, invalidHeaders)
	...
	for _, invalidBody := range []string{
		...
		response := perform(handler, http.MethodPost, "/api/v2/mcp/grant-requests/"+service.item.ID+"/approve", invalidBody, validHeaders)
		assert.Equal(t, http.StatusBadRequest, response.Code, response.Body.String())
	}
	...
	for _, test := range []struct {
		...
		response := perform(handler, http.MethodGet, "/api/v2/mcp/grant-requests/"+service.item.ID, "", headers)
		assert.Equal(t, test.status, response.Code, test.err)
		assert.Contains(t, response.Body.String(), test.code, test.err)
	}
	missingRoute := perform(handler, http.MethodGet, "/api/v2/mcp/grant-requests/"+service.item.ID+"/approve/extra", "", headers)
...
func TestGrantRequestSessionOriginAndCSRFAreEnforced(t *testing.T) {
	...
	listed := perform(handler, http.MethodGet, "/api/v2/mcp/grant-requests", "", sessionHeaders)
	assert.Equal(t, http.StatusOK, listed.Code, listed.Body.String())
	body := `{"reason":"not_approved"}`
	missingCSRF := perform(handler, http.MethodPost, "/api/v2/mcp/grant-requests/"+service.item.ID+"/reject", body, map[string]string{
	...
	missingOrigin := perform(handler, http.MethodGet, "/api/v2/mcp/grant-requests", "", map[string]string{"Cookie": contract.SessionCookieName + "=session"})
