...
func (service *fakeServerService) Inventory(context.Context, *int64) ([]serverdomain.Server, int64, error) {
	items := []serverdomain.Server{}
	if service.server.ID != "" {
		items = append(items, service.server)
	}
...
func (service *fakeServerService) ActiveOperations(context.Context, string) ([]serverdomain.Operation, bool, error) {
	return append([]serverdomain.Operation(nil), service.operations...), false, nil
}

func (service *fakeServerService) QueryOperations(_ context.Context, _ string, _ serverdomain.OperationQuery, _ *serverdomain.OperationQueryCursor, _ int) (serverdomain.OperationQueryPage, error) {
	return serverdomain.OperationQueryPage{Items: append([]serverdomain.Operation(nil), service.operations...), CollectionRange: contract.CollectionRange{TotalCount: len(service.operations)}}, nil
}
...
func TestServerCreateReplayGetAndDisplayPatch(t *testing.T) {
	...
	created := perform(handler, http.MethodPost, "/api/v2/mcp/servers", body, headers)
	require.Equal(t, http.StatusCreated, created.Code, created.Body.String())
	assert.Equal(t, contract.ServerETag(testID, "1"), created.Header().Get("ETag"))
	assert.Equal(t, "/api/v2/mcp/servers/"+testID, created.Header().Get("Location"))
	assert.Contains(t, created.Body.String(), `"operation":null`)
	assert.Equal(t, testID, service.create.Idempotency.AuthorityID)

	replay := perform(handler, http.MethodPost, "/api/v2/mcp/servers", body, headers)
	require.Equal(t, http.StatusOK, replay.Code, replay.Body.String())

	got := perform(handler, http.MethodGet, "/api/v2/mcp/servers/"+testID, "", map[string]string{"Authorization": "Bearer " + testBearer})
	...
	patch := perform(handler, http.MethodPatch, "/api/v2/mcp/servers/"+testID, `{"display_name":"Renamed"}`, map[string]string{"Authorization": "Bearer " + testBearer, "Content-Type": contract.MediaTypeJSON, "If-Match": contract.ServerETag(testID, "1")})
...
func TestServerMutationsTriggerReconciliationOnlyForBehavioralWork(t *testing.T) {
	...
	display := perform(displayHandler, http.MethodPatch, "/api/v2/mcp/servers/"+testID, `{"display_name":"Renamed"}`, map[string]string{"Authorization": "Bearer " + testBearer, "Content-Type": contract.MediaTypeJSON, "If-Match": contract.ServerETag(testID, "1")})
	...
	behavioral := perform(behaviorHandler, http.MethodPatch, "/api/v2/mcp/servers/"+testID, `{"enabled":true}`, map[string]string{"Authorization": "Bearer " + testBearer, "Content-Type": contract.MediaTypeJSON, "If-Match": contract.ServerETag(testID, "1")})
...
func TestServerOperationCreateReplayReadAndList(t *testing.T) {
	...
	created := perform(handler, http.MethodPost, "/api/v2/mcp/servers/"+testID+"/operations", `{"kind":"reload"}`, headers)
	...
	replayed := perform(handler, http.MethodPost, "/api/v2/mcp/servers/"+testID+"/operations", `{"kind":"reload"}`, headers)
	require.Equal(t, http.StatusOK, replayed.Code, replayed.Body.String())

	operationID := service.operations[0].ID
	got := perform(handler, http.MethodGet, "/api/v2/mcp/servers/"+testID+"/operations/"+operationID, "", map[string]string{"Authorization": "Bearer " + testBearer})
	require.Equal(t, http.StatusOK, got.Code, got.Body.String())
	listed := perform(handler, http.MethodGet, "/api/v2/mcp/servers/"+testID+"/operations?limit=1", "", map[string]string{"Authorization": "Bearer " + testBearer})
	require.Equal(t, http.StatusOK, listed.Code, listed.Body.String())
	assert.Contains(t, listed.Body.String(), operationID)

	invalid := perform(handler, http.MethodPost, "/api/v2/mcp/servers/"+testID+"/operations", `{"kind":"activate"}`, headers)
	assert.Equal(t, http.StatusBadRequest, invalid.Code)
	assert.Contains(t, invalid.Body.String(), "invalid_operation")
	wrongParent := perform(handler, http.MethodGet, "/api/v2/mcp/servers/01ARZ3NDEKTSV4RRFFQ69G5FAX/operations/"+operationID, "", map[string]string{"Authorization": "Bearer " + testBearer})
...
func TestServerCreateReportsKnownFieldTypeContext(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			response := perform(newServerTestHandler(t, new(fakeServerService)), http.MethodPost, "/api/v2/mcp/servers", test.body, map[string]string{"Authorization": "Bearer " + testBearer, "Content-Type": contract.MediaTypeJSON, "Idempotency-Key": "known-field"})
...
func TestServerCreateReportsRepositoryConfigurationContextWithoutReflection(t *testing.T) {
	...
	t.Run("closed context", func(t *testing.T) {
		...
		response := perform(newServerTestHandler(t, service), http.MethodPost, "/api/v2/mcp/servers", validBody, map[string]string{"Authorization": "Bearer " + testBearer, "Content-Type": contract.MediaTypeJSON, "Idempotency-Key": "repository-context"})
		require.Equal(t, http.StatusBadRequest, response.Code)
	...
	t.Run("invalid internal context", func(t *testing.T) {
		...
		response := perform(newServerTestHandler(t, service), http.MethodPost, "/api/v2/mcp/servers", validBody, map[string]string{"Authorization": "Bearer " + testBearer, "Content-Type": contract.MediaTypeJSON, "Idempotency-Key": "repository-fallback"})
		require.Equal(t, http.StatusBadRequest, response.Code)
...
func TestServerCreateRequiresCompleteConfiguration(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			service := new(fakeServerService)
			handler := newServerTestHandler(t, service)
			response := perform(handler, http.MethodPost, "/api/v2/mcp/servers", test.body, map[string]string{"Authorization": "Bearer " + testBearer, "Content-Type": contract.MediaTypeJSON, "Idempotency-Key": "complete"})
			require.Equal(t, http.StatusBadRequest, response.Code)
			var problem contract.ServerConfigurationProblemEnvelope
			require.NoError(t, json.Unmarshal(response.Body.Bytes(), &problem))
...
func TestServerRequestValidationAndPreconditions(t *testing.T) {
	...
	missingPrecondition := perform(handler, http.MethodPatch, "/api/v2/mcp/servers/"+testID, `{"display_name":"Renamed"}`, authJSON)
	assert.Equal(t, http.StatusPreconditionRequired, missingPrecondition.Code)
	stale := perform(handler, http.MethodPatch, "/api/v2/mcp/servers/"+testID, `{"display_name":"Renamed"}`, map[string]string{"Authorization": "Bearer " + testBearer, "Content-Type": contract.MediaTypeJSON, "If-Match": `W/` + contract.ServerETag(testID, "1")})
