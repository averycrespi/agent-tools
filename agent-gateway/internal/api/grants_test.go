...
func TestGrantCreateListGetDeleteAndInvalidation(t *testing.T) {
	...
	created := perform(handler, http.MethodPost, "/api/v2/mcp/grants", `{"description":"Test grant","principal_id":"`+testID+`","effect":"deny","server_id":"`+testServerID+`","upstream_name":"danger","constraint":{"equals":{"/count":1.0}},"expires_at":"2027-08-25T00:00:00Z"}`, headers)
	...
	listed := perform(handler, http.MethodGet, "/api/v2/mcp/grants?limit=1&principal_id="+testID+"&server_id="+testServerID, "", map[string]string{"Authorization": "Bearer " + testBearer})
	...
	continued := perform(handler, http.MethodGet, "/api/v2/mcp/grants?limit=2&principal_id="+testID+"&server_id="+testServerID+"&cursor="+*page.NextCursor, "", map[string]string{"Authorization": "Bearer " + testBearer})
	...
	got := perform(handler, http.MethodGet, "/api/v2/mcp/grants/"+service.grants[0].ID, "", map[string]string{"Authorization": "Bearer " + testBearer})
	require.Equal(t, http.StatusOK, got.Code, got.Body.String())
	assert.Equal(t, contract.GrantETag(service.grants[0].ID, "1"), got.Header().Get("ETag"))
	updated := perform(handler, http.MethodPatch, "/api/v2/mcp/grants/"+service.grants[0].ID, `{"description":"Updated access"}`, map[string]string{
	...
	deleted := perform(handler, http.MethodDelete, "/api/v2/mcp/grants/"+second.ID, "", map[string]string{"Authorization": "Bearer " + testBearer})
...
func TestGrantPatchRequiresExactPreconditionAndClosedDescriptionBody(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			response := perform(handler, http.MethodPatch, "/api/v2/mcp/grants/"+testID, test.body, headers)
	...
	response := perform(handler, http.MethodPatch, "/api/v2/mcp/grants/"+testID, `{"description":"changed"}`, stale)
...
func TestGrantCreateRequiresAllMembersAndExactNullableShapes(t *testing.T) {
	...
	response := perform(handler, http.MethodPost, "/api/v2/mcp/grants", valid, headers)
	...
	for _, test := range []struct{ name, body, code string }{
		...
		t.Run(test.name, func(t *testing.T) {
			result := perform(handler, http.MethodPost, "/api/v2/mcp/grants", test.body, headers)
...
func TestGrantConstraintValidationUsesProductionCompilerWithoutMutation(t *testing.T) {
	...
	valid := perform(handler, http.MethodPost, "/api/v2/mcp/grant-constraints/validate", `{"constraint":{"version":2,"regex":{"/resource":"[a-z]+/[0-9]+"}}}`, headers)
	require.Equal(t, http.StatusOK, valid.Code, valid.Body.String())
	assert.JSONEq(t, `{"valid":true,"diagnostics":[]}`, valid.Body.String())

	invalid := perform(handler, http.MethodPost, "/api/v2/mcp/grant-constraints/validate", `{"constraint":{"version":2,"regex":{"/resource":"["}}}`, headers)
	...
	invalidRoot := perform(handler, http.MethodPost, "/api/v2/mcp/grant-constraints/validate", `{"constraint":{"version":3,"equals":{"/x":true}}}`, headers)
...
func TestGrantAuthenticationSessionAndTargetValidation(t *testing.T) {
	service := &fakePrincipalService{}
	handler := newGrantHandler(t, service, allowGrantTarget, nil)
	unauthenticated := perform(handler, http.MethodGet, "/api/v2/mcp/grants", "", nil)
	...
	session := perform(handler, http.MethodPost, "/api/v2/mcp/grants", body, map[string]string{"Cookie": contract.SessionCookieName + "=session", "Origin": contract.CanonicalOrigin, "X-CSRF-Token": "csrf", "Content-Type": contract.MediaTypeJSON})
	assert.Equal(t, http.StatusCreated, session.Code, session.Body.String())
	missingOrigin := perform(handler, http.MethodPost, "/api/v2/mcp/grants", body, map[string]string{"Cookie": contract.SessionCookieName + "=session", "X-CSRF-Token": "csrf", "Content-Type": contract.MediaTypeJSON})
	...
	invalidTarget := perform(rejecting, http.MethodPost, "/api/v2/mcp/grants", body, map[string]string{"Authorization": "Bearer " + testBearer, "Content-Type": contract.MediaTypeJSON})
...
func TestGrantQueryValidationErrorsAndNoFailureInvalidation(t *testing.T) {
	...
	for _, path := range []string{
		"/api/v2/mcp/grants?unknown=x", "/api/v2/mcp/grants?limit=1&limit=2", "/api/v2/mcp/grants?principal_id=", "/api/v2/mcp/grants?server_id=null", "/api/v2/mcp/grants?cursor=%ZZ",
	} {
	...
	badCursor := perform(handler, http.MethodGet, "/api/v2/mcp/grants?cursor=abc", "", map[string]string{"Authorization": "Bearer " + testBearer})
	...
	for _, test := range []struct {
		...
		{authorization.ErrInvalidInput, 400, "invalid_grant", http.MethodPost, "/api/v2/mcp/grants", `{"description":"Test grant","principal_id":"` + testID + `","effect":"allow","server_id":"` + testServerID + `","upstream_name":null,"constraint":null,"expires_at":null}`},
		{authorization.ErrResourceLimit, 429, "resource_limit", http.MethodPost, "/api/v2/mcp/grants", `{"description":"Test grant","principal_id":"` + testID + `","effect":"allow","server_id":"` + testServerID + `","upstream_name":null,"constraint":null,"expires_at":null}`},
		{authorization.ErrStaleCursor, 409, "stale_cursor", http.MethodGet, "/api/v2/mcp/grants", ""},
		{authorization.ErrNotFound, 404, "not_found", http.MethodDelete, "/api/v2/mcp/grants/01ARZ3NDEKTSV4RRFFQ69G5FAY", ""},
		{authorization.ErrStorageUnavailable, 503, "authorization_unavailable", http.MethodGet, "/api/v2/mcp/grants", ""},
		{errors.New("foreign"), 503, "authorization_unavailable", http.MethodGet, "/api/v2/mcp/grants", ""},
	} {
