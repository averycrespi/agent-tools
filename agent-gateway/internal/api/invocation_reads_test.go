...
func TestInvocationReadAPI(t *testing.T) {
	...
	listed := perform(handler, http.MethodGet, "/api/v2/mcp/invocations?"+query.Encode(), "", bearer)
	...
	item := perform(handler, http.MethodGet, "/api/v2/mcp/invocations/"+service.item.ID, "", bearer)
	...
	assert.Equal(t, http.StatusOK, perform(handler, http.MethodGet, "/api/v2/mcp/invocations", "", session).Code)
	assert.Equal(t, contract.AdminListPageDefault, service.query.Limit)
	for _, target := range []string{"/api/v2/mcp/invocations", "/api/v2/mcp/invocations/" + service.item.ID} {
		assert.Equal(t, http.StatusUnauthorized, perform(handler, http.MethodGet, target, "", nil).Code)
		assert.Equal(t, http.StatusForbidden, perform(handler, http.MethodGet, target, "", map[string]string{"Cookie": contract.SessionCookieName + "=session"}).Code)
		assert.Equal(t, http.StatusOK, perform(handler, http.MethodGet, target, "", session).Code)
		for _, method := range []string{http.MethodPost, http.MethodPut, http.MethodPatch, http.MethodDelete} {
			response := perform(handler, method, target, "", bearer)
			assert.Equal(t, http.StatusMethodNotAllowed, response.Code)
			assert.Equal(t, http.MethodGet, response.Header().Get("Allow"))
		}
	}
	for _, target := range []string{"/api/v2/invocations", "/api/v2/invocations/" + service.item.ID} {
		for _, auth := range []map[string]string{nil, bearer, session} {
			for _, method := range []string{http.MethodGet, http.MethodPost} {
				response := perform(handler, method, target, "", auth)
	...
	for _, target := range []string{
		"/api/v2/mcp/invocations?unknown=x", "/api/v2/mcp/invocations?limit=1&limit=2", "/api/v2/mcp/invocations?cursor=",
		"/api/v2/mcp/invocations?principal_id=null", "/api/v2/mcp/invocations?limit=0", "/api/v2/mcp/invocations?limit=101",
		"/api/v2/mcp/invocations?admission_class=other", "/api/v2/mcp/invocations?decision=other", "/api/v2/mcp/invocations?outcome=other",
	} {
	...
	for _, decision := range []string{"allow", "deny", "block", "not_evaluated"} {
		response := perform(handler, http.MethodGet, "/api/v2/mcp/invocations?decision="+decision, "", bearer)
	...
	for _, outcome := range contract.InvocationOutcomeClasses() {
		response := perform(handler, http.MethodGet, "/api/v2/mcp/invocations?outcome="+string(outcome), "", bearer)
		assert.Equal(t, http.StatusOK, response.Code)
		assert.Equal(t, outcome, *service.query.Filters.Outcome)
	}
	literalResponse := perform(handler, http.MethodGet, "/api/v2/mcp/invocations?tool=null&principal=null", "", bearer)
	assert.Equal(t, http.StatusOK, literalResponse.Code)
	assert.Equal(t, "null", service.query.Filters.Tool)
	tooLongCursor := perform(handler, http.MethodGet, "/api/v2/mcp/invocations?cursor="+strings.Repeat("x", 513), "", bearer)
	assert.Equal(t, http.StatusBadRequest, tooLongCursor.Code)
	assert.Contains(t, tooLongCursor.Body.String(), "invalid_cursor")
	assert.Equal(t, http.StatusBadRequest, perform(handler, http.MethodGet, "/api/v2/mcp/invocations", `{}`, bearer).Code)
	assert.Equal(t, http.StatusBadRequest, perform(handler, http.MethodGet, "/api/v2/mcp/invocations/"+service.item.ID+"?x=1", "", bearer).Code)
	assert.Equal(t, http.StatusBadRequest, perform(handler, http.MethodGet, "/api/v2/mcp/invocations/"+service.item.ID, `{}`, bearer).Code)
	assert.Equal(t, http.StatusNotFound, perform(handler, http.MethodGet, "/api/v2/mcp/invocations/"+service.item.ID+"/extra", "", bearer).Code)
	response := perform(handler, http.MethodPost, "/api/v2/mcp/invocations", "", bearer)
	assert.Equal(t, http.StatusMethodNotAllowed, response.Code)
	assert.Equal(t, http.MethodGet, response.Header().Get("Allow"))
