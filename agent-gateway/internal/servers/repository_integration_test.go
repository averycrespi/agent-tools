...
func TestServerSnapshotWatermarkExcludesLaterInsert(t *testing.T) {
	...
	inventory, upper, err := repository.Inventory(context.Background(), nil)
	require.NoError(t, err)
	require.Len(t, inventory, 2)
	mustCreateServer(t, repository, "snapshot-later", false)
	pinned, pinnedUpper, err := repository.Inventory(context.Background(), &upper)
	require.NoError(t, err)
	assert.Equal(t, inventory, pinned)
	assert.Equal(t, upper, pinnedUpper)
	fresh, _, err := repository.Inventory(context.Background(), nil)
	require.NoError(t, err)
