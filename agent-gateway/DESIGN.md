...
| 5   | Over-broad rules                  | Operator writes `host = "**"` or `no_intercept_hosts = ["*.com"]`, silently disabling MITM or scoping               | Hard rejection of unambiguous cases (wildcard-only `no_intercept_hosts`, public-suffix `no_intercept_hosts`, missing `match.host`). Soft warning for legal-but-suspicious `host = "**"` surfaces at load and in `rules check`   | `internal/config/validate.go` `validateNoInterceptHosts`; `internal/rules/parse.go` `decodeMatchBlock`                             |
