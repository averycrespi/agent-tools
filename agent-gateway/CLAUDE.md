...
- Online CLI uses one selected administrator bearer and only `internal/controlclient`; no prompt/argv/environment fallback. `--data-dir` selects credentials, never private storage/keyring/domain access. Keep separate stdout/stderr, typed exits, strict input, exact ETags, prepared one-time sinks, confirmation, and no automatic replay.
