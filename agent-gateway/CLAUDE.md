...
Use [the disposable demo](docs/maintainers/frontend-development.md#use-a-disposable-feature-branch-gateway) for isolated feature testing; that guide owns setup, datasets, credentials and shutdown. Demo evidence qualifies neither native keyring nor production persistence.
...
- Follow [installation safety](docs/operators/installation-safety.md): ignore legacy paths and tombstones for implicit selection, retain all historical artifacts, and preserve safety checks on the selected explicit/default root. The migrator is retired; never clear tombstones, reservations or recovery markers as naming cleanup. LaunchAgent commands remain bounded to five seconds/1 MiB, retain child identity through cleanup and never replay mutations.
...
- Follow the [table conventions](docs/design/browser-control-plane.md#table-conventions) and shared [implementation contract](docs/maintainers/frontend-development.md#table-implementation).
