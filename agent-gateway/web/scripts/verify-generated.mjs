...
const tracked = join(repositoryRoot, "agent-gateway/internal/api/static");
const temporaryRoot = mkdtempSync(join(tmpdir(), "agent-gateway-ui-"));
  ...
  for (const name of ["first", "second"]) {
    ...
    execFileSync(
      ...
      [
        ...
        "build",
        "--config",
        "agent-gateway/web/vite.config.ts",
        "--outDir",
        outDir,
        "--emptyOutDir",
      ],
