...
export function parseDevelopmentConfig(
  ...
  for (const retired of ["MCP_GATEWAY_UI_LISTEN", "MCP_GATEWAY_UI_GATEWAY"]) {
    if (Object.hasOwn(environment, retired)) {
      throw new Error(
        `${retired} is retired; use ${retired.replace("MCP_GATEWAY_", "AGENT_GATEWAY_")}`,
      );
    }
  }
  if (args.length !== 0) {
  ...
  const listen = parseAuthority(
    environment.AGENT_GATEWAY_UI_LISTEN ?? DEFAULT_LISTEN,
    "AGENT_GATEWAY_UI_LISTEN",
  );
  const gatewayValue = environment.AGENT_GATEWAY_UI_GATEWAY ?? DEFAULT_GATEWAY;
  if (!gatewayValue.startsWith("http://")) {
    throw new Error(
      "AGENT_GATEWAY_UI_GATEWAY must be a canonical http://127/8 URL with an explicit port",
    );
  }
...
export async function runDevelopmentServer(
  ...
  const developmentRoot = await mkdtemp(
    resolve(tmpdir(), "agent-gateway-ui-development-"),
  );
