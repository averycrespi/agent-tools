...
function launch(
  ...
  const child = spawn(process.execPath, [entryPoint, ...args], {
    ...
    env: {
      ...process.env,
      AGENT_GATEWAY_UI_LISTEN: listenAuthority,
      AGENT_GATEWAY_UI_GATEWAY: gateway,
      AGENT_GATEWAY_UI_SECRET_CANARY: "must-not-appear",
    },
...
async function developmentTemporaryRoots(): Promise<string[]> {
  return (await readdir(tmpdir(), { withFileTypes: true }))
    .filter(
      (entry) =>
        entry.isDirectory() &&
        entry.name.startsWith("agent-gateway-ui-development-"),
    )
...
async function staticDigest(): Promise<string> {
  const staticRoot = resolve(
    repositoryRoot,
    "agent-gateway/internal/api/static",
  );
...
test("selector defaults and canonical loopback overrides", () => {
  ...
  assert.deepEqual(
    parseDevelopmentConfig(
      {
        AGENT_GATEWAY_UI_LISTEN: "127.12.34.56:49152",
        AGENT_GATEWAY_UI_GATEWAY: "http://127.255.0.1:65535",
      },
      [],
    ),
    {
...
for (const value of [
  ...
  test(`selector rejects invalid frontend authority ${value}`, () => {
    assert.throws(
      () => parseDevelopmentConfig({ AGENT_GATEWAY_UI_LISTEN: value }, []),
      /AGENT_GATEWAY_UI_LISTEN/,
    );
  });
}
...
for (const value of [
  ...
  test(`selector rejects invalid Gateway target ${value}`, () => {
    assert.throws(
      () => parseDevelopmentConfig({ AGENT_GATEWAY_UI_GATEWAY: value }, []),
      /AGENT_GATEWAY_UI_GATEWAY/,
    );
  });
}

for (const retired of ["MCP_GATEWAY_UI_LISTEN", "MCP_GATEWAY_UI_GATEWAY"]) {
  test(`startup rejects retired setting ${retired} without leaking values`, () => {
    const replacement = retired.replace("MCP_GATEWAY_", "AGENT_GATEWAY_");
    for (const value of ["", "private-canary"]) {
      assert.throws(
        () =>
          parseDevelopmentConfig(
            { [retired]: value, [replacement]: "new" },
            [],
          ),
        { message: `${retired} is retired; use ${replacement}` },
      );
    }
    const result = spawnSync(process.execPath, [entryPoint], {
      cwd: repositoryRoot,
      env: { ...process.env, [retired]: "private-canary" },
      encoding: "utf8",
      timeout: 5_000,
      maxBuffer: 64 * 1024,
    });
    ...
    assert.match(
      result.stderr,
      new RegExp(`${retired} is retired; use ${replacement}`),
    );
...
test("provisioning exports are not retired development settings", () => {
  assert.deepEqual(
    parseDevelopmentConfig(
      {
        MCP_GATEWAY_ENDPOINT: "client",
        MCP_GATEWAY_AGENT_TOKEN: "private-canary",
      },
      [],
    ),
    parseDevelopmentConfig({}, []),
...
test("source server serves authored modules and owns HMR without workspace output", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      await readFile(
        resolve(repositoryRoot, "agent-gateway/web/public/favicon.svg"),
      ),
    );
