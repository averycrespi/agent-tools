...
function launch(
  ...
  const child = spawn(process.execPath, [entryPoint], {
    ...
    env: {
      ...process.env,
      AGENT_GATEWAY_UI_LISTEN: `127.0.0.1:${frontendPort}`,
      AGENT_GATEWAY_UI_GATEWAY: `http://127.0.0.1:${gatewayPort}`,
    },
...
test("contract projection matches the authoritative API body bound", () => {
  const result = spawnSync(
    ...
    { cwd: resolve(repositoryRoot, "agent-gateway"), encoding: "utf8" },
  );
