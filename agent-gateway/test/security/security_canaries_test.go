...
import (
	...
	"github.com/averycrespi/agent-tools/agent-gateway/test/acceptance"
...
func TestDiagnosticSinkConstructionHasOneProductionOwner(t *testing.T) {
	...
	require.NoError(t, filepath.Walk(root, func(path string, info os.FileInfo, walkErr error) error {
		...
		for _, imported := range parsed.Imports {
			if imported.Path.Value != `"github.com/averycrespi/agent-tools/agent-gateway/internal/diagnostics"` {
				continue
			}
			alias := "diagnostics"
			...
			for _, declaration := range parsed.Decls {
				ast.Inspect(declaration, func(node ast.Node) bool {
					...
					constructors++
					require.Equal(t, "cmd/agent-gateway/root.go", filepath.ToSlash(relative))
...
func TestSecurityEvidenceOwnerManifest(t *testing.T) {
	...
	webRoot := filepath.Join(root, "agent-gateway", "web", "src")
...
func TestStaticSecretSinkClosure(t *testing.T) {
	...
	for _, base := range []string{"cmd", "internal"} {
		require.NoError(t, filepath.Walk(filepath.Join(repositoryRoot(t), "agent-gateway", base), func(path string, info os.FileInfo, walkErr error) error {
			...
			if strings.Contains(string(contents), "/api/v2/mcp/invocations") {
			...
			for _, imported := range parsed.Imports {
				if imported.Path.Value == `"log"` || imported.Path.Value == `"log/slog"` {
					relative := filepath.ToSlash(strings.TrimPrefix(path, filepath.Join(repositoryRoot(t), "agent-gateway")+string(filepath.Separator)))
