...
func runCLIGrantRequestInputMatrix(t *testing.T) {
	...
	approveBody := `{"description":"Approved access","approved_policy":{"scope":"tool","target":"request-target.echo","constraint":null,"duration_seconds":null,"future_tools_acknowledged":false}}`
	require.NoError(t, os.WriteFile(approvePath, []byte(approveBody), 0o600))
	require.NoError(t, os.WriteFile(invalidPath, []byte(`{"description":"Approved access","approved_policy":{"scope":"server","target":"mcp_gateway","constraint":null,"duration_seconds":null,"future_tools_acknowledged":true,"unknown":true}}`), 0o600))
	results := make([]testutil.ProcessResult, 0, 10)

	listed := runOnlineCLI(t, harness, bearerPath, true, "mcp", "grant-request", "list", "--principal-id", principal.Resource.ID, "--state", "pending", "--limit", "10", "--output", "json")
	got := runOnlineCLI(t, harness, bearerPath, true, "mcp", "grant-request", "get", first.ID)
	...
	invalid := runOnlineCLI(t, harness, bearerPath, false, "mcp", "grant-request", "approve", first.ID, "--etag", firstHandle.ETag, "--file", invalidPath, "--yes", "--output", "json")
	oldReject := runOnlineCLI(t, harness, bearerPath, false, "mcp", "grant-request", "reject", second.ID, "--file", invalidPath, "--yes", "--output", "json")
	results = append(results, invalid, oldReject)
	assert.Equal(t, 2, invalid.ExitCode)
	assert.Equal(t, 2, oldReject.ExitCode)
	refused := runOnlineCLI(t, harness, bearerPath, false, "mcp", "grant-request", "approve", first.ID, "--etag", firstHandle.ETag, "--file", approvePath, "--output", "json")
	results = append(results, refused)
	assert.Equal(t, 2, refused.ExitCode)
	approved := runOnlineCLI(t, harness, bearerPath, true, "mcp", "grant-request", "approve", first.ID, "--etag", firstHandle.ETag, "--file", approvePath, "--yes", "--output", "json")
	...
	directApproved := runOnlineCLI(t, harness, bearerPath, true, "mcp", "grant-request", "approve", fourth.ID, "--description", "Approved access", "--scope", "tool", "--target", "request-target.echo", "--yes", "--output", "json")
	...
	stale := runOnlineCLI(t, harness, bearerPath, false, "mcp", "grant-request", "reject", first.ID, "--etag", firstHandle.ETag, "--reason", "scope_too_broad", "--yes", "--output", "json")
	terminal := runOnlineCLI(t, harness, bearerPath, false, "mcp", "grant-request", "approve", first.ID, "--etag", contract.GrantRequestETag(first.ID, approvedRequest.Revision), "--file", approvePath, "--yes", "--output", "json")
	results = append(results, stale, terminal)
	assert.Equal(t, 5, stale.ExitCode)
	assert.Equal(t, 5, terminal.ExitCode)

	rejected := runOnlineCLI(t, harness, bearerPath, true, "mcp", "grant-request", "reject", second.ID, "--etag", secondHandle.ETag, "--reason", "scope_too_broad", "--yes", "--output", "json")
	...
	defer fake.Close()
	uncertain := runCLIAt(t, harness, bearerPath, fake.URL, "mcp", "grant-request", "reject", third.ID, "--etag", thirdHandle.ETag, "--reason", "scope_too_broad", "--yes", "--output", "json")
	...
	preHandoff := runOnlineCLI(t, harness, bearerPath, false, "mcp", "grant-request", "get", third.ID, "--output", "json")
