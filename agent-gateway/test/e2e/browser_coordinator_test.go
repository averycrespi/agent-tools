...
func assertOrdinaryBinaryNeedsNoNode(t *testing.T) {
	...
	ordinaryBinary := filepath.Join(temporaryRoot, "agent-gateway")
	goBinary, err := exec.LookPath("go")
	require.NoError(t, err)
	runner, err := testutil.NewBinaryRunner(30*time.Second, 64*1024)
	require.NoError(t, err)
	moduleRoot := filepath.Dir(browserWebRoot(t))
	built, err := runner.Run(context.Background(), goBinary, "-C", moduleRoot, "build", "-o", ordinaryBinary, "./cmd/agent-gateway")
