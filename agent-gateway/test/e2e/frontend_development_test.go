...
func TestFrontendDevelopmentLiveReload(t *testing.T) {
	...
	staticBefore := directoryDigest(t, filepath.Join(repositoryRoot, "agent-gateway", "internal", "api", "static"))
	...
	copyDirectory(t, filepath.Join(repositoryRoot, "agent-gateway", "web"), fixtureWeb)
	...
	devProcess, err := devRunner.Start(
		context.Background(),
		envBinary,
		"AGENT_GATEWAY_UI_LISTEN="+frontendAuthority,
		"AGENT_GATEWAY_UI_GATEWAY=http://"+harness.authority,
		"node",
	...
	assert.Equal(t, staticBefore, directoryDigest(t, filepath.Join(repositoryRoot, "agent-gateway", "internal", "api", "static")))
...
func developmentTemporaryRoots(t *testing.T) []string {
	t.Helper()
	matches, err := filepath.Glob(filepath.Join(os.TempDir(), "agent-gateway-ui-development-*"))
	require.NoError(t, err)
