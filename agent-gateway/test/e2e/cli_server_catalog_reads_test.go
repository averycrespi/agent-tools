...
func TestCLIServerCatalogReads(t *testing.T) {
	...
	serverList := runOnlineCLI(t, harness, bearerPath, true, "mcp", "server", "list", "--limit", "1", "--output", "json")
	results = append(results, serverList)
	serverAPI := harness.adminSnapshot(http.MethodGet, "/api/v2/mcp/servers?limit=1", nil)
	...
	serverNext := runOnlineCLI(t, harness, bearerPath, true, "mcp", "server", "list", "--limit", "1", "--cursor", *servers.NextCursor, "--output", "json")
	...
	serverGet := runOnlineCLI(t, harness, bearerPath, true, "mcp", "server", "get", alpha.ServerID, "--output", "json")
	results = append(results, serverGet)
	serverGetAPI := harness.adminSnapshot(http.MethodGet, "/api/v2/mcp/servers/"+alpha.ServerID, nil)
	assert.JSONEq(t, string(serverGetAPI.Body), string(serverGet.Stdout))
	serverTable := runOnlineCLI(t, harness, bearerPath, true, "mcp", "server", "get", alpha.ServerID)
	...
	descriptorList := runOnlineCLI(t, harness, bearerPath, true, "mcp", "server", "descriptor", "list", alpha.ServerID, "--limit", "1", "--retired", "include", "--output", "json")
	results = append(results, descriptorList)
	descriptorAPI := harness.adminSnapshot(http.MethodGet, "/api/v2/mcp/servers/"+alpha.ServerID+"/descriptors?limit=1", nil)
	...
	descriptorGet := runOnlineCLI(t, harness, bearerPath, true, "mcp", "server", "descriptor", "get", alpha.ServerID, descriptor.ID, "--output", "json")
	results = append(results, descriptorGet)
	descriptorGetAPI := harness.adminSnapshot(http.MethodGet, "/api/v2/mcp/servers/"+alpha.ServerID+"/descriptors/"+descriptor.ID, nil)
	assert.JSONEq(t, string(descriptorGetAPI.Body), string(descriptorGet.Stdout))
	descriptorTable := runOnlineCLI(t, harness, bearerPath, true, "mcp", "server", "descriptor", "list", alpha.ServerID, "--limit", "1", "--retired", "exclude")
	...
	catalogList := runOnlineCLI(t, harness, bearerPath, true, "mcp", "catalog", "list", "--limit", "1", "--output", "json")
	results = append(results, catalogList)
	catalogTable := runOnlineCLI(t, harness, bearerPath, true, "mcp", "catalog", "list", "--limit", "1")
	...
	catalogAPI := harness.adminSnapshot(http.MethodGet, "/api/v2/mcp/catalog?limit=1", nil)
	...
	stale := runOnlineCLI(t, harness, bearerPath, false, "mcp", "catalog", "list", "--cursor", staleCursor, "--output", "json")
	...
	invalid := runOnlineCLI(t, harness, bearerPath, false, "mcp", "server", "get", "invalid", "--output", "json")
	results = append(results, invalid)
	assert.Equal(t, 2, invalid.ExitCode)
	assert.Empty(t, invalid.Stdout)
	invalidRetired := runOnlineCLI(t, harness, bearerPath, false, "mcp", "server", "descriptor", "list", alpha.ServerID, "--retired", "invalid", "--output", "json")
