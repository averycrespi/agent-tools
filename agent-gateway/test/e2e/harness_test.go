...
func prepareGatewayBinary() error {
	...
	var err error
	gatewayBuildRoot, err = os.MkdirTemp("", "agent-gateway-e2e-")
	if err != nil {
		return fmt.Errorf("create E2E build root: %w", err)
	}
	gatewayBinaryPath = filepath.Join(gatewayBuildRoot, "agent-gateway")
	runner, err := testutil.NewBinaryRunner(30*time.Second, 64*1024)
	if err != nil {
		return fmt.Errorf("create E2E build runner: %w", err)
	}
	result, err := runner.Run(context.Background(), "go", "-C", moduleRoot, "build", "-tags=e2e", "-o", gatewayBinaryPath, "./cmd/agent-gateway")
	if err != nil {
