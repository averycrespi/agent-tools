...
func TestReleaseExecutorCancellationReapsNestedProcessGroup(t *testing.T) {
	...
	t.Setenv("AGENT_GATEWAY_ACCEPTANCE_EXECUTOR_FIXTURE", pidPath)
...
func TestReleaseExecutorCLIHandlesTERM(t *testing.T) {
	...
	require.NoError(t, err)
	built, err := runner.Run(context.Background(), "go", "-C", filepath.Join(repositoryRoot(t), "agent-gateway"), "build", "-o", binary, "./test/acceptance/cmd")
	require.NoError(t, err, "build stderr: %s", built.Stderr)
	readyPath := filepath.Join(t.TempDir(), "ready")
	t.Setenv("AGENT_GATEWAY_ACCEPTANCE_SIGNAL_READY", readyPath)
...
func TestReleaseExecutorNestedFixture(t *testing.T) {
	pidPath := os.Getenv("AGENT_GATEWAY_ACCEPTANCE_EXECUTOR_FIXTURE")
	if pidPath == "" {
		return
	}
