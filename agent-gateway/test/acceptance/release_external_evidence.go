...
func releaseExternalEvidenceDefinitions(root string) ([]releaseExternalEvidenceDefinition, error) {
	contents, err := os.ReadFile(filepath.Join(root, "agent-gateway", "web", "environments.json"))
	if err != nil {
		return nil, err
	}
	var manifest struct {
...
func validateReleaseExternalSchema(contents []byte) error {
	...
	if err := compiler.AddResource("urn:agent-gateway:release-external-evidence", document); err != nil {
		return err
	}
	schema, err := compiler.Compile("urn:agent-gateway:release-external-evidence")
	if err != nil {
		return err
	}
	var instance any
