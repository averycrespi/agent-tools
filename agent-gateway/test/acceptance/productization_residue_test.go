...
func TestPublishedMaintainerSurfacesContainNoImplementationProgramResidue(t *testing.T) {
	...
	var paths []string
	for _, relative := range []string{"README.md", "CLAUDE.md", "Makefile", "agent-gateway/README.md", "agent-gateway/DESIGN.md", "agent-gateway/CLAUDE.md", "agent-gateway/Makefile"} {
		paths = append(paths, filepath.Join(root, relative))
	}
	docs, err := filepath.Glob(filepath.Join(root, "agent-gateway", "docs", "*.md"))
	require.NoError(t, err)
	paths = append(paths, docs...)
	for _, relative := range []string{
		"agent-gateway/internal/contract/control_plane_capabilities.go", "agent-gateway/internal/contract/documentation_ownership.go", "agent-gateway/internal/contract/product_behavior_manifest.go",
		"agent-gateway/test/acceptance/acceptance.go", "agent-gateway/test/acceptance/purpose_dag.go", "agent-gateway/test/acceptance/release_external_evidence.go",
		"agent-gateway/test/acceptance/release_profile.go", "agent-gateway/test/acceptance/release_report.go", "agent-gateway/test/acceptance/release_runner.go", "agent-gateway/test/acceptance/cmd/main.go",
	} {
		paths = append(paths, filepath.Join(root, relative))
	}
...
func TestTrackedGoTestNamesContainNoImplementationProgramOwners(t *testing.T) {
	root := filepath.Join(repositoryRoot(t), "agent-gateway")
