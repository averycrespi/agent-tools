...
func TestFrontendDevelopmentDocumentation(t *testing.T) {
	...
	require.NoError(t, err)
	require.Contains(t, string(rootClaude), "agent-gateway/docs/maintainers/frontend-development.md")
	...
	require.NoError(t, err)
	require.Contains(t, string(packageManifest), `"ui:dev": "node agent-gateway/web/dev-server.ts"`)
