...
func TestPurposeEvidenceDAGMetadataIsComplete(t *testing.T) {
	...
	for id, leaf := range dag.Leaves {
		...
		assert.Equal(t, []string{"make", "-C", "agent-gateway", id}, rootCommand.Argv, id)
	...
	assert.Equal(t, 35, dag.Leaves["test-browser-workflows"].GatewayStarts)
	assert.Equal(t, 34, dag.Leaves["test-browser-workflows"].BrowserStarts)
...
func TestPurposeEvidenceDAGExpansionAndMultiplicity(t *testing.T) {
	...
	assert.Equal(t, 1, countPurposeCommandsContaining(commands, "agent-gateway/web/scripts/verify-generated.mjs"))
...
func TestPurposeEvidenceDAGMatchesMakeAndNPMDefinitions(t *testing.T) {
	...
	for id, leaf := range dag.Leaves {
		...
		for index, dependency := range dependencies {
			...
			expected = strings.ReplaceAll(expected, "$(AGENT_GATEWAY_STRESS_COUNT)", strconv.Itoa(leaf.Repeats))
