...
func TestPurposeNamedRootForwardingAndOwnership(t *testing.T) {
	...
	for _, target := range forwarded {
		assert.Equal(t, "\t$(MAKE) -C agent-gateway "+target, purposeMakeTargetRecipe(makefile, target), target)
		...
		assert.Contains(t, string(output), "make -C agent-gateway "+target, target)
	}
	assert.Equal(t, "\t$(MAKE) -C agent-gateway accept REPORT=\"$(REPORT)\"", purposeMakeTargetRecipe(makefile, "accept"))
	assert.Equal(t, "\t$(MAKE) -C agent-gateway adopt-acceptance-report REPORT=\"$(REPORT)\" ADOPTION=\"$(ADOPTION)\"", purposeMakeTargetRecipe(makefile, "adopt-acceptance-report"))
	assert.Equal(t, "\t$(MAKE) -C agent-gateway qualify-external-evidence", purposeMakeTargetRecipe(makefile, "qualify-external-evidence"))
	assert.NotContains(t, purposeMakeTargetRecipe(makefile, "check-other-tools"), "agent-gateway")
	assert.Contains(t, makefile, "OTHER_TOOLS := $(filter-out agent-gateway,$(TOOLS))")
}
...
func TestPurposeNamedStructuredOutputIsOptIn(t *testing.T) {
	...
	for _, target := range []string{"test-unit", "test-integration", "test-harness", "test-material", "test-browser", "test-e2e", "test-security", "test-frontend-development-browser", "frontend-verify-supply-chain"} {
		t.Run(target, func(t *testing.T) {
			...
			output := purposeTargetDryRun(t, root, target, "AGENT_GATEWAY_TEST_JSON=1")
			found := false
...
func purposeTargetDryRun(t *testing.T, root, target string, overrides ...string) string {
	...
	require.NoError(t, err)
	arguments := append([]string{"-n", "-C", root, target, "AGENT_GATEWAY_TEST_JSON=0"}, overrides...)
