...
func TestSuiteJSONOutputPreservesExecutionAndFailure(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			t.Setenv("AGENT_GATEWAY_SUITE_JSON_HELPER", root)
			t.Setenv("GOWORK", "off")
...
func TestRetiredEnvironmentStopsAcceptanceBeforeSelection(t *testing.T) {
	t.Setenv("MCP_GATEWAY_ACCEPTANCE_WAIT_FOR_SIGNAL", "")
	t.Setenv("AGENT_GATEWAY_ACCEPTANCE_WAIT_FOR_SIGNAL", "1")
	require.Equal(t, 2, run([]string{"suite-inventory"}))
}

func TestSuiteJSONSubprocess(t *testing.T) {
	root := os.Getenv("AGENT_GATEWAY_SUITE_JSON_HELPER")
	if root == "" {
...
func TestAcceptanceTemporaryRootsDetectsRootsCreatedDuringRun(t *testing.T) {
	...
	require.NoError(t, err)
	created, err := os.MkdirTemp("", "agent-gateway-ui-development-")
	require.NoError(t, err)
