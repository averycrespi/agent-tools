...
func testFreshUserDocumentationGraph(t *testing.T) {
	...
	assert.Contains(t, gatewayOverview, "(agent-gateway/README.md)")
...
func testOperationalGuidesCoverBehaviorManifest(t *testing.T) {
	...
	markers := map[string]string{
		...
		"product.invocation.bounded_retention":              "rolling bounded traffic history",
		"product.invocation.polling_without_authority":      "Neither event handling nor repeated operator reads submit, complete, resume, or replay a call",
	}
	seen := 0
...
func TestMaintainerGuidanceAndReleaseDocumentation(t *testing.T) {
	...
	for _, link := range []string{"docs/maintainers/frontend-development.md", "docs/maintainers/release-verification.md"} {
		...
		assert.Contains(t, rootGuidance, "agent-gateway/"+link)
	}
