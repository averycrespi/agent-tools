...
func purposeEvidenceDAG() purposeEvidenceGraph {
	...
	leaves := map[string]purposeEvidenceLeaf{
		...
		"test-browser-workflows":     leaf("test-browser-workflows", []string{"tier.browser.workflows", "product.interface.developer_first", "product.browser.authority_recovery"}, 5*time.Minute, 6*time.Minute, 1, 35, 34, []string{"browser workflow output", "browser cleanup records"}, "agent-gateway/test/e2e/harness_test.go"),
