...
func purposeEvidenceDAG() purposeEvidenceGraph {
	...
	leaves := map[string]purposeEvidenceLeaf{
		...
		"test-browser-workflows":     leaf("test-browser-workflows", []string{"tier.browser.workflows", "product.interface.developer_first", "product.browser.authority_recovery"}, 3*time.Minute, 210*time.Second, 1, 34, 34, []string{"browser workflow output", "browser cleanup records"}, "agent-gateway/test/e2e/harness_test.go"),
		...
		"test-frontend-development-node":    leaf("test-frontend-development-node", []string{"frontend.command_interface", "frontend.proxy_boundary"}, 30*time.Second, 45*time.Second, 1, 0, 0, []string{"Node development proxy and audit contract matrix output"}, packageManifest, "agent-gateway/web/dev-server.ts", "agent-gateway/web/dev-proxy.ts", "agent-gateway/web/tests/dev-server.test.ts", "agent-gateway/web/tests/proxy-admission.test.ts", "agent-gateway/web/tests/audit-contract.test.ts", "agent-gateway/web/src/audit-contract.ts"),
	...
	addCommand("node.test-dev", []string{"node", "--test", "--test-concurrency=1", "--test-timeout=30000"}, []string{packageManifest, "agent-gateway/web/tests/dev-server.test.ts", "agent-gateway/web/tests/proxy-admission.test.ts", "agent-gateway/web/tests/audit-contract.test.ts", "agent-gateway/web/src/audit-contract.ts"}, nil)
