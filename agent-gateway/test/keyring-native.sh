...
# Reject before material tests or native setup, even when a retired value is empty.
for setting in ${!MCP_GATEWAY_@}; do
	case "$setting" in
	MCP_GATEWAY_ENDPOINT | MCP_GATEWAY_AGENT_TOKEN) continue ;;
	esac
	printf '%s is retired; use AGENT_GATEWAY_%s\n' "$setting" "${setting#MCP_GATEWAY_}" >&2
	exit 2
done
...
if [[ "${AGENT_GATEWAY_KEYRING_NATIVE_SELF_TEST:-}" == "1" ]]; then
	case "${AGENT_GATEWAY_KEYRING_NATIVE_FORCE_RESULT:-}" in
...
run_native_test() {
	AGENT_GATEWAY_KEYRING_NATIVE=1 go -C "$ROOT" run ./test/acceptance/cmd run-suite test-keyring-native >"$LOG" 2>&1
}
...
run_linux() {
	...
	dbus-run-session -- env \
		...
		AGENT_GATEWAY_NATIVE_ROOT="$ROOT" \
		bash -euo pipefail -c '
			eval "$(printf "%s\n" agent-gateway-test | gnome-keyring-daemon --unlock --components=secrets)"
			trap '\''if [[ -n "${GNOME_KEYRING_PID:-}" ]]; then kill "$GNOME_KEYRING_PID" 2>/dev/null || true; fi'\'' EXIT
			AGENT_GATEWAY_KEYRING_NATIVE=1 go -C "$AGENT_GATEWAY_NATIVE_ROOT" run ./test/acceptance/cmd run-suite test-keyring-native
		' >"$LOG" 2>&1
	status=$?
	set -e
	if [[ $status -eq 0 ]]; then
...
run_darwin() {
	if [[ "${AGENT_GATEWAY_DISPOSABLE_MACOS_KEYCHAIN:-}" != "1" ]]; then
		emit skipped macos_disposable_keychain_required passed skipped
	fi
	local password status=0
	DARWIN_SANDBOX=$(mktemp -d)
	DARWIN_KEYCHAIN="$DARWIN_SANDBOX/agent-gateway-test.keychain-db"
	password=agent-gateway-test
