...
func Parse(contents []byte) (Result, error) {
	...
	if err := compiler.AddResource("urn:agent-gateway:keyring-native-result", schemaDocument); err != nil {
		return Result{}, fmt.Errorf("load native evidence schema: %w", err)
	}
	schema, err := compiler.Compile("urn:agent-gateway:keyring-native-result")
	if err != nil {
