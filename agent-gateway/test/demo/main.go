...
func run(ctx context.Context, listen, dataset string, out io.Writer, opts options) (result error) {
	...
	root, err := os.MkdirTemp("", "agent-gateway-demo-")
	if err != nil {
	...
	fmt.Fprintf(&output, "Separate Vite: AGENT_GATEWAY_UI_GATEWAY=http://%s npm run ui:dev\nStop with Ctrl-C; all temporary state will be removed. Relaunch for fresh data; AGENT_GATEWAY_DEMO_DATASET=empty for empty state.\n", listen)
