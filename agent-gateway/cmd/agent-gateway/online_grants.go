...
func grantListPath(options *onlineOptions) (string, error) {
	...
	return controlclient.BuildListPath("/api/v2/mcp/grants", controlclient.ListOptions{Limit: options.limit, Cursor: options.cursor, Filters: filters, AllowedFilters: []string{"principal_id", "server_id"}})
}
...
func runGrantCreateRequest(command *cobra.Command, options *onlineOptions, body []byte) error {
	...
	response, err := client.Do(command.Context(), controlclient.Request{Method: http.MethodPost, Path: "/api/v2/mcp/grants", Header: header, Body: body})
	if err != nil {
...
func runGrantUpdate(command *cobra.Command, options *onlineOptions, args []string) error {
	...
	response, err := client.Do(command.Context(), controlclient.Request{Method: http.MethodPatch, Path: "/api/v2/mcp/grants/" + args[0], Header: header, Body: options.intent.body})
	if err != nil {
		return writeOnlineFailure(command, options.output, controlclient.ClassifyClientError(err))
	}
...
func runGrantDelete(command *cobra.Command, options *onlineOptions, args []string) error {
	...
	response, err := client.Do(command.Context(), controlclient.Request{Method: http.MethodDelete, Path: "/api/v2/mcp/grants/" + args[0], Header: header})
	if err != nil {
		failure := controlclient.ClassifyClientError(err)
		if failure.Code == "client_outcome_uncertain" {
