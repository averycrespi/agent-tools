...
func TestIndexIncludesExplorerUI(t *testing.T) {
	...
	require.Contains(t, body, "grid-template-columns:max-content minmax(0,1fr)")
	require.Contains(t, body, ".grid>.muted{white-space:nowrap}")
	...
	require.Contains(t, body, "toLocaleDateString(undefined,{year:'numeric',month:'numeric',day:'numeric'})+' '+d.toLocaleTimeString(undefined,{hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false})")
	require.NotContains(t, body, "toLocaleString()")
	require.Contains(t, body, "task_id:d.task.id")
	require.Contains(t, body, "run_id:r.id||''")
	require.Contains(t, body, "started_at:fmt(r.started_at)")
	require.Contains(t, body, "updated_at:fmt(d.task.updated_at)")
	require.Contains(t, body, "completed_at:fmt(r.ended_at)")
	require.Less(t, strings.Index(body, "attempt:r.attempt||''"), strings.Index(body, "started_at:fmt(r.started_at)"))
	require.Less(t, strings.Index(body, "started_at:fmt(r.started_at)"), strings.Index(body, "updated_at:fmt(d.task.updated_at)"))
	require.Less(t, strings.Index(body, "updated_at:fmt(d.task.updated_at)"), strings.Index(body, "completed_at:fmt(r.ended_at)"))
	require.Less(t, strings.Index(body, "completed_at:fmt(r.ended_at)"), strings.Index(body, "supervisor_pid:r.supervisor_pid||''"))
