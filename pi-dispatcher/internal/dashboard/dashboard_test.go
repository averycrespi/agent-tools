...
func TestIndexIncludesExplorerUI(t *testing.T) {
	...
	require.Contains(t, compactBody, "toLocaleDateString(undefined,{year:'numeric',month:'numeric',day:'numeric'")
	require.Contains(t, compactBody, "toLocaleTimeString(undefined,{hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false")
	require.NotContains(t, body, "toLocaleString()")
	require.Contains(t, compactBody, "task_id:d.task.id")
	require.Contains(t, compactBody, "run_id:r.id||''")
	require.Contains(t, compactBody, "started_at:fmt(r.started_at)")
	require.Contains(t, compactBody, "updated_at:fmt(d.task.updated_at)")
	require.Contains(t, compactBody, "completed_at:fmt(r.ended_at)")
	require.Contains(t, compactBody, "max_duration:formatDuration(r.max_duration_seconds)")
	require.Less(t, strings.Index(compactBody, "attempt:r.attempt||''"), strings.Index(compactBody, "started_at:fmt(r.started_at)"))
	require.Less(t, strings.Index(compactBody, "started_at:fmt(r.started_at)"), strings.Index(compactBody, "updated_at:fmt(d.task.updated_at)"))
	require.Less(t, strings.Index(compactBody, "updated_at:fmt(d.task.updated_at)"), strings.Index(compactBody, "completed_at:fmt(r.ended_at)"))
	require.Less(t, strings.Index(compactBody, "completed_at:fmt(r.ended_at)"), strings.Index(compactBody, "supervisor_pid:r.supervisor_pid||''"))
	require.NotContains(t, body, "run:r.id||''")
	require.NotContains(t, body, "$('detail-title').textContent=state.selected")
	require.Contains(t, compactBody, "pi_output_logs:r.stdout_log_path||''")
	require.Contains(t, compactBody, "pi_error_logs:r.stderr_log_path||''")
	require.Contains(t, compactBody, "pi_events:r.pi_events_path||''")
	require.Contains(t, compactBody, "agent_provider:agent.provider||''")
	require.Contains(t, compactBody, "agent_model:agent.model||''")
	require.Contains(t, compactBody, "agent_thinking:agent.thinking||''")
	require.Contains(t, compactBody, "agent_tools:formatList(agent.tools)")
	require.Contains(t, compactBody, "env:formatList(r.env_var_names)")
	require.Contains(t, compactBody, "Object.entries(meta).filter(([,v])=>v!==''&&v!==undefined&&v!==null)")
	...
	require.Less(t, strings.Index(compactBody, "pi_output_logs:r.stdout_log_path||''"), strings.Index(compactBody, "pi_error_logs:r.stderr_log_path||''"))
	require.Less(t, strings.Index(compactBody, "pi_error_logs:r.stderr_log_path||''"), strings.Index(compactBody, "pi_events:r.pi_events_path||''"))
