...
func TestXDGOverrides(t *testing.T) {
	...
	cases := []struct {
		...
		{"AgentTokenFile", paths.AgentTokenFile(), "/c/http-broker/agent-token"},
		{"AdminTokenFile", paths.AdminTokenFile(), "/c/http-broker/admin-token"},
		{"LegacyTokenFile", paths.LegacyTokenFile(), "/c/http-broker/auth-token"},
		{"TokenLockFile", paths.TokenLockFile(), "/c/http-broker/.token.lock"},
