...
Repository-owned guest provisioning is retired. Through a trusted, authenticated transfer channel, copy only `~/.config/http-broker/agent-token` and the public `~/.local/share/http-broker/ca.pem` to the client environment. Never transfer `admin-token`, `ca.key`, upstream credentials or the host data directory. On upgrade, use the canonical agent file rather than migration-only `auth-token`; migration preserves the agent value.

Verify ownership and reject symlinked parents/files or unexplained existing destinations before transfer. Keep the token in an owner-private directory (`0700`) and regular file (`0600` or `0400`). Export the public certificate with `http-broker ca export` and verify its provenance before installing it in the client trust store. Public `/ca.pem` remains available, but an unauthenticated network fetch alone cannot establish trust. Never disable TLS verification.

Configure the client proxy at `http://host.lima.internal:8220` for trusted local Lima forwarding (or host loopback for local clients), with agent-only Basic or Bearer proxy authentication. For clients requiring a proxy URL, the supported form is `http://x:<agent-token>@host.lima.internal:8220`; construct it privately from the current token at client startup, never put it in arguments, logs or persisted shell literals. Disable tracing and avoid environment dumps. Set the client-supported `HTTP_PROXY`/`HTTPS_PROXY` (and lowercase equivalents where needed); environment delivery exposes the token to inheriting processes. Keep the host listeners loopback-only.

Install the authenticated public CA using the client OS’s trust-store procedure. Runtimes may additionally need `NODE_EXTRA_CA_CERTS`, `SSL_CERT_FILE`, `REQUESTS_CA_BUNDLE`, `CURL_CA_BUNDLE`, `GIT_SSL_CAINFO` or `AWS_CA_BUNDLE` pointing to a bundle containing the required public roots. Do not overwrite a system bundle with only the interception CA.

Configure appropriate `NO_PROXY`/`no_proxy` exclusions for local services (commonly `localhost,127.0.0.1,::1,host.lima.internal`) and verify the actual client’s routing. Proxy-variable behavior differs by runtime; no repository script or test now guarantees a generated bypass list. Preserve unrelated profile content when reconciling old managed blocks. These instructions do not modify installed profiles or qualify live adoption.
...
**There is no overlap window.** Clients trusting only the old CA cannot verify new interception certificates. Securely transfer and install the new public CA in every client trust store, then send `SIGHUP` to activate it in a running proxy. Retire old trust only after reconciling affected clients; never transfer the private key.
...
Agent rotation is a coordinated cutover, not zero-downtime revocation: rotate the host file, securely refresh client agent-token files while avoiding new client starts, send `SIGHUP` promptly, then reconnect clients with the old value. New CONNECT and absolute-form requests reject the old credential after activation; traffic inside established tunnel or MITM CONNECT connections continues.
...
Client proxy and bypass behavior is runtime- and version-dependent, including Node’s undici. After manual setup, confirm that host MCP services remain reachable from the actual client while intended external traffic uses the proxy. The retired script-derived neighbour-reachability test is not evidence for manual client configuration.
