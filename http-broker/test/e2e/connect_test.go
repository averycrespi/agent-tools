...
func TestProxyRequiresAuthentication(t *testing.T) {
	...
	if got := resp.Header.Get("X-Http-Broker-Reason"); got != "proxy-auth-required" {
...
func TestFallthroughDeny(t *testing.T) {
	...
	if got := resp.Header.Get("X-Http-Broker-Reason"); got != "fallthrough-deny" {
...
func TestNetguard(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := resp.Header.Get("X-Http-Broker-Reason"); got != "blocked-address" {
