...
- The current S2 checkpoint implements the closed vocabulary, shared strict JSON, durable server/operation/idempotency authority foundation, transaction-fenced server-scoped keyring publication/invalidation, desired-server plus explicit-operation APIs, transport-neutral runtime reconciliation/retry scheduling, the bounded direct stdio supervisor, and injected stop-before-start/publication ordering. The supervisor is not yet production activation: do not infer concrete process cleanup, network work, OAuth orchestration, catalog traversal, concrete downstream activation, routing, invocation, or product-UI behavior; add behavior only in its owning task.
