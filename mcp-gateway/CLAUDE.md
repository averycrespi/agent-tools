...
- `internal/oauth` owns the exact registered-resource trust anchor, RFC 9728 endpoint/root discovery, issuer selection, RFC 8414/OpenID order, strict extensible metadata projection, and canonical explicit trusted-origin policy. A present challenge metadata URL never silently falls back; public reachability never selects an issuer; cross-origin or multiple issuers require the exact desired pin. Only the resource origin and explicit trusted DNS origins may permit restricted machine-fetch addresses. Keep registration, flows, tokens, refresh, and revocation out until their owning tasks.
