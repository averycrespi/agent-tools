...
- `internal/downstream` owns raw JSON-RPC IDs/envelopes, exact outbound MCP application headers, bounded JSON/SSE bodies, stdio/HTTP connection closure, exact modern/legacy/auto negotiation, fresh verified fallback, and runtime-local immutable legacy sessions. Fixed downstream identity is `mcp-gateway/s2`. Modern evidence never downgrades; modern HTTP is sessionless; legacy session loss closes the runtime. Do not use SDK `Client.Connect`, `CommandTransport`, automatic cancellation/retry/reconnect, cached list paths, subscriptions, or list-change machinery.
