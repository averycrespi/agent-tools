...
- `internal/downstream` owns raw JSON-RPC IDs/envelopes, exact outbound MCP application headers, bounded JSON/SSE bodies, and stdio/HTTP connection closure. Do not use SDK `Client.Connect`, `CommandTransport`, automatic cancellation/retry/reconnect, or cached list paths.
- The official MCP SDK sits behind Gateway-owned authentication, classification, binding, limits, and lifecycle.
- The current S2 checkpoint implements the closed vocabulary, shared strict JSON, durable server/operation/idempotency authority foundation, transaction-fenced server-scoped keyring publication/invalidation, desired-server plus explicit-operation APIs, transport-neutral runtime reconciliation/retry scheduling, the bounded direct stdio supervisor, injected stop-before-start/publication ordering, and concurrent deadline-bounded all-runtime drain. The supervisor is not yet production activation: verified process cleanup is implemented, but do not infer network work, OAuth orchestration, catalog traversal, concrete downstream activation, routing, invocation, or product-UI behavior; add behavior only in its owning task.
