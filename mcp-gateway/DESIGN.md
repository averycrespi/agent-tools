...
The current checkpoint deliberately stops at S2 production activation and internal catalog routing. Exact static, bearer, and OAuth authority is acquired only for the dependent runtime; credential-free transports skip keyring access, and provider capability or admission failures stay isolated to that server. The composition supports stdio and hardened Streamable HTTP with modern `2026-07-28`, legacy `2025-11-25`, and strict auto fallback, plus refresh/revocation/disconnect and catalog publication. It does not implement principal/grant management, agent credential issuance, a production capability consumer, tool invocation or audit, grant-request tools, or product UI pages. Pre-Streamable HTTP+SSE and MCP list-change notifications remain excluded.
