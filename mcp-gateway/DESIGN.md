...
MCP Gateway is a single-user local service that provides a strict HTTP/MCP boundary and durable control foundation. S1 established the accepted service foundation. The current S2 foundation adds the closed contract, shared strict-JSON primitive, durable server authority repository, transaction-fenced server keyring publication/invalidation, authenticated desired-server and operation APIs, and one production runtime/catalog composition. It activates validated stdio or hardened Streamable HTTP runtimes, negotiates the selected protocol era, traverses catalogs, and publishes internal active routes through one process-local registry. Production MCP ingress remains deny-all and consumes none of those routes; S3 now has an internal principal/default-grant authority foundation, while its control API and positive authentication plus invocation/audit and product UI workflows remain later slices.
...
The final S2.1 acceptance runner is a clean-checkpoint gate rather than another behavior owner. It composes focused race and source/canary/slice/documentation/backup checks, integration and real-binary lifecycle suites, the module audit, an unsuppressed vulnerability scan, native classification, the repository-wide check, and whitespace validation. It stops on the first failure and emits one closed schema-validated report binding the result to the exact Git SHA and required-clean state, with command-to-acceptance-criterion and artifact mappings plus the nested native result. Starting dirty, modifying tracked state during checks, malformed native output, or native `failed` blocks acceptance; native `skipped` is retained as an additive platform gap.
...
One repository-owned nonblocking authority gate encloses authentication and lease registration as a single linearized operation. Success returns an already-registered pending lease with immutable safe binding, latch/drain-aware currentness, cancellation completion, and idempotent release; the registry creates no goroutine, timer, queue, or alternate owner. Drain fences new authentication before waiting boundedly for the gate, then removes and cancels every pending lease outside the registry lock. A timed-out drain leaves the fence set for a later completion. The admitted phase is reserved for T16's post-commit detachment and has no public transition yet.

Ingress authentication runs before MCP body reads, era classification, and session lookup. Production still uses a deny-all authenticator; positive fixtures inject opaque principal/credential/expiry bindings. Registered S3 leases remain unwired until the coordinated ingress and production cutover checkpoints.
...
The executable exposes stopped-process `initialize`, `admin-reset`, `restore --verify-current`, and verified backup replacement, plus `serve` for the verified HTTP/control and production runtime/catalog composition. Public health, authenticated admin credential/session/status/backup/event, desired-server, operation, auth-flow, descriptor, and active-catalog resources, the minimal static shell, bounded OAuth callback and token lifecycle, dual-era MCP adapters, composed fixed limits, and coordinated shutdown are implemented. `executeServe` constructs the mandatory composition before opening the listener, wires API status and triggers to that graph's actual owners, marks readiness, and then starts reconstruction. Desired mutations and operations notify its manager, which validates exact authority, activates concrete stdio or hardened HTTP runtimes, negotiates modern/legacy/auto, traverses catalogs, and publishes one authoritative internal registry. Server-specific downstream failure remains isolated after readiness. Production `/mcp` remains deny-all and consumes no internal capability.

## Non-goals

The current executable checkpoint deliberately retains S2 production activation and internal catalog routing. Exact static, bearer, and OAuth authority is acquired only for the dependent runtime; credential-free transports skip keyring access, and provider capability or admission failures stay isolated to that server. The composition supports stdio and hardened Streamable HTTP with modern `2026-07-28`, legacy `2025-11-25`, and strict auto fallback, plus refresh/revocation/disconnect and catalog publication. S3 principal/default-grant authority exists only behind its internal repository and is not yet exposed through the control API or positive MCP authentication. Agent credential issuance, a production capability consumer, tool invocation or audit, grant-request tools, and product UI pages remain absent. Pre-Streamable HTTP+SSE and MCP list-change notifications remain excluded.
