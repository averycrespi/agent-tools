...
func TestInitialOAuthTokenPostCommitFailureLeavesNoAuthorityAndFailsFlow(t *testing.T) {
	...
	require.NotNil(t, stored.Diagnostic)
	assert.Equal(t, flow.Flow.ID, stored.Diagnostic.CorrelationID)
	assert.Equal(t, contract.OAuthDiagnosticCredentialInstallation, stored.Diagnostic.Stage)
	assert.Equal(t, contract.ReasonOAuthRejected, stored.Diagnostic.Reason)
	assert.Nil(t, stored.Diagnostic.HTTPStatus)
	authority, err := repository.Authority(context.Background(), server.ID)
	require.NoError(t, err)
