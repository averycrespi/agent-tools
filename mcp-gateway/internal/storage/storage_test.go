...
func TestInitializeCreatesVerifiedGatewayDatabase(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, expectedMigrationVersions(), versions)
...
func TestOpenMigratesCommittedKeyringSchemaFixture(t *testing.T) {
	ctx := context.Background()
	ownership := newOwnership(t)
	copyFixture(t, "testdata/schema-v2.db", ownership.Layout().Database)
...
func TestOpenMigratesAcceptedS1SchemaThreeFixture(t *testing.T) {
	...
	assert.Equal(t, CurrentSchema, identity.SchemaVersion)
	assert.Equal(t, expectedMigrationVersions(), mustMigrationVersions(t, store, ctx))
	for _, table := range []string{"keyring_authority_fences", "server_identities", "servers", "server_credentials", "server_oauth_registrations", "server_operations", "server_operation_watermarks", "server_auth_flows", "server_auth_flow_watermarks", "server_catalogs", "server_catalog_issues", "durable_tool_identities", "tool_descriptors", "s2_idempotency"} {
		var actual string
		require.NoError(t, store.database.QueryRowContext(ctx, `
			SELECT name FROM sqlite_schema WHERE type = 'table' AND name = ?`, table).Scan(&actual))
		assert.Equal(t, table, actual)
...
func expectedMigrationVersions() []int {
	versions := make([]int, CurrentSchema)
