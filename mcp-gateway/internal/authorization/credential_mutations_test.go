...
func TestCredentialEntropyIdentityAndKnownRollbackFailuresPreserveAuthority(t *testing.T) {
	...
	shortBearer, err := New(store, &fixedClock{now: testNow}, bytes.NewReader(make([]byte, 32+31)))
	require.NoError(t, err)
	_, err = shortBearer.IssueCredential(context.Background(), principal.ID, "1")
	assert.ErrorIs(t, err, ErrStorageUnavailable)
	shortID, err := New(store, &fixedClock{now: testNow}, bytes.NewReader(make([]byte, 32+32+9)))
	require.NoError(t, err)
	_, err = shortID.IssueCredential(context.Background(), principal.ID, "1")
	assert.ErrorIs(t, err, ErrStorageUnavailable)
	reservedID, err := New(store, &fixedClock{now: unixEpochClock{}.Now()}, bytes.NewReader(make([]byte, 32+42)))
	require.NoError(t, err)
	...
	identityRepository, err := New(store, &fixedClock{now: testNow}, bytes.NewReader(append(make([]byte, 32), idEntropy...)))
	require.NoError(t, err)
	...
	collisionRepository, err := New(store, &fixedClock{now: testNow}, bytes.NewReader(append(make([]byte, 32), collisionEntropy...)))
	require.NoError(t, err)
	...
	repeatedBearer, err := New(store, &fixedClock{now: testNow}, bytes.NewReader(append(make([]byte, 32), repeatedBearerEntropy...)))
	require.NoError(t, err)
