...
func (repository *Repository) compileSource(source string) (CompiledConstraint, error) {
	if repository.constraintCompiler != nil {
		return repository.constraintCompiler([]byte(source))
	}
	return CompileConstraint([]byte(source))
}

func (repository *Repository) compileReadConstraint(source string) (CompiledConstraint, error) {
	return repository.compileConstraint("", source)
}
