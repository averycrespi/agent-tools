...
func authorityMutationCases() []authorityMutationCase {
	return []authorityMutationCase{
		...
		{name: "grant create", prepare: credentialMutationCase(func(repository *Repository, credential contract.AgentCredentialCreation) error {
			_, err := repository.CreateGrant(context.Background(), CreateGrantRequest{Description: stringPointer("Test grant"), PrincipalID: credential.Principal.ID, Effect: contract.GrantAllow, Target: accesstarget.MCP{ServerID: id(51)}}, allowCurrentTarget)
			return err
		})},
