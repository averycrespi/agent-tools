...
func activityEvidenceViolations(source productionSource) []string {
	...
	const activityPackage = "github.com/averycrespi/agent-tools/mcp-gateway/internal/activity"
	for _, imported := range source.imports {
		if strings.HasPrefix(source.path, "internal/activity/") && imported != "github.com/averycrespi/agent-tools/mcp-gateway/internal/contract" {
...
func TestActivityEvidenceOwnershipNegativeFixtures(t *testing.T) {
	for _, fixture := range []struct{ path, contents string }{
		...
		{"internal/audit/bad.go", "package audit\nimport \"github.com/averycrespi/agent-tools/mcp-gateway/internal/activity\"\nvar _ activity.Envelope\n"},
