...
var controlPlaneCapabilities = []ControlPlaneCapability{
	...
	{ID: "grant-create", Operation: "Grant create", WebControl: "Access / Grants / Create", CLIUses: []string{"grant create --name NAME --principal-id ID --effect EFFECT --server-id ID [--upstream-name NAME] [--expires-at RFC3339] [--file PATH]"}, Mechanics: "required name; exclusive direct unconstrained/file complete body"},
	...
	{ID: "grant-request-approve", Operation: "Grant-request approve", WebControl: "request detail / Review", CLIUses: []string{"grant-request approve REQUEST_ID --name NAME --scope SCOPE --target TARGET [--etag ETAG] [--duration-seconds SECONDS] [--acknowledge-future-tools] [--file PATH]"}, Mechanics: "required grant name; exclusive direct/file narrowing; automatic or explicit ETag; confirmation; no replay"},
