...
var controlPlaneCapabilities = []ControlPlaneCapability{
	...
	{ID: "grant-create", Operation: "Grant create", WebControl: "Access / Grants / Create", CLIUses: []string{"grant create --principal-id ID --effect EFFECT --server-id ID [--description TEXT] [--upstream-name NAME] [--expires-at RFC3339] [--read-only] [--file PATH]"}, Mechanics: "optional description; exclusive direct unconstrained/file complete body"},
	{ID: "grant-update", Operation: "Grant metadata update", WebControl: "grant detail / Description", CLIUses: []string{"grant update ID --description TEXT [--etag ETAG]"}, Mechanics: "description only; automatic or explicit ETag; no replay"},
	...
	{ID: "grant-request-approve", Operation: "Grant-request approve", WebControl: "request detail / Review", CLIUses: []string{"grant-request approve REQUEST_ID --scope SCOPE --target TARGET [--description TEXT] [--etag ETAG] [--duration-seconds SECONDS] [--acknowledge-future-tools] [--read-only] [--file PATH]"}, Mechanics: "optional grant description; exclusive direct/file narrowing; automatic or explicit ETag; confirmation; no replay"},
