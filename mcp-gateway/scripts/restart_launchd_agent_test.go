...
case "$1" in
  print)
    ...
    if [ "$RESTART_MODE" = post-stop-error ] && [ -f "$RESTART_STOPPED" ]; then echo 'permission denied' >&2; exit 1; fi
    if { [ "$RESTART_MODE" = delayed-removal ] || [ "$RESTART_MODE" = delayed-removal-process-stuck ]; } && [ -f "$RESTART_STOPPED" ] && [ ! -f "$RESTART_STOPPED.observed" ]; then
      touch "$RESTART_STOPPED.observed"
      echo '  pid = 12345'
      exit 0
    fi
...
`)
			writeTool("ps", `
printf 'ps\n' >> "$RESTART_CALLS"
case "$RESTART_MODE" in process-stuck|delayed-removal-process-stuck) exit 0 ;; process-error) exit 2 ;; esac
exit 1
`)
