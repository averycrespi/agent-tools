...
func TestCLIGrantCreateInputModes(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		_, _ = response.Write([]byte(`{"id":"` + resourceID + `","principal_id":"` + resourceID + `","effect":"allow","server_id":"` + resourceID + `","upstream_name":null,"constraint":null,"expires_at":null,"state":"active","created_at":"2026-08-30T00:00:00Z"}`))
	}))
	defer server.Close()
	...
	directArgs := []string{"grant", "create", "--principal-id", resourceID, "--effect", "allow", "--server-id", resourceID}
	baseBody := `{"principal_id":"` + resourceID + `","effect":"allow","server_id":"` + resourceID + `","upstream_name":null,"constraint":null,"expires_at":null}`
	...
	require.NoError(t, err, "%s", output)
	assert.JSONEq(t, `{"principal_id":"`+resourceID+`","effect":"allow","server_id":"`+resourceID+`","upstream_name":"example_tool","constraint":null,"expires_at":"`+expiresAt+`"}`, string(<-requests))

	constrainedBody := `{"principal_id":"` + resourceID + `","effect":"deny","server_id":"` + resourceID + `","upstream_name":"example_tool","constraint":{"equals":{"/region":"us"}},"expires_at":null}`
	...
	for _, test := range []struct {
		...
		{name: "missing principal", args: []string{"grant", "create", "--effect", "allow", "--server-id", resourceID}},
		{name: "invalid effect", args: []string{"grant", "create", "--principal-id", resourceID, "--effect", "audit", "--server-id", resourceID}},
		{name: "invalid server", args: []string{"grant", "create", "--principal-id", resourceID, "--effect", "allow", "--server-id", "bad"}},
	...
	for _, flag := range []string{"principal-id", "effect", "server-id", "upstream-name", "expires-at", "file"} {
