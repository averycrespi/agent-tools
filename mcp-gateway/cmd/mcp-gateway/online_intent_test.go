...
func TestCLILocalIntentPrecedesAuthority(t *testing.T) {
	...
	declarations := map[string][]string{
		...
		"grant create":            {"principal-id", "effect", "server-id", "upstream-name", "expires-at"},
		"grant-request approve":   {"scope", "target", "duration-seconds", "acknowledge-future-tools"},
	...
	body, err = grantSpec.buildBody(map[string]string{"principal-id": id, "effect": "allow", "server-id": id}, nil, map[string]bool{"principal-id": true, "effect": true, "server-id": true})
	require.NoError(t, err)
	assert.JSONEq(t, `{"principal_id":"01ARZ3NDEKTSV4RRFFQ69G5FAV","effect":"allow","server_id":"01ARZ3NDEKTSV4RRFFQ69G5FAV","upstream_name":null,"constraint":null,"expires_at":null}`, string(body))
}
