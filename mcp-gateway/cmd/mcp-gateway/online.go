...
func onlineCommandSpecs() []onlineCommandSpec {
	return []onlineCommandSpec{
		...
		onlineSpec([]string{"server", "operation", "start"}, "start ID", "server operation start ID --etag ETAG --kind KIND", "etag", "kind", "idempotency-key", "yes"),
...
var onlineLeafDescriptions = map[string]string{
	...
	"server operation start ID --etag ETAG --kind KIND":            "Request a server operation",
...
var onlineRequiredFlags = map[string][]string{
	...
	"server update ID --etag ETAG [--display-name NAME] [--enable|--disable] [--file PATH]": {"etag"},
	"server delete ID --etag ETAG":                                                                    {"etag"},
	"server operation start ID --etag ETAG --kind KIND":                                               {"etag"},
