...
Grant list/create/read/delete use the same sole authority and a servers-owned supplied-transaction target callback. Creation requires a non-null `name` plus every policy member, including nullable `upstream_name`, `constraint`, and `expires_at`; names are valid UTF-8, 1–256 bytes, free of control characters and surrounding whitespace, and need not be unique; listing accepts singleton principal/server filters bound into its insertion-watermark cursor.
...
Atomic approval is authorization-owned: its nonqueueing authority gate encloses one storage mutation, current owner/DENY/grant-capacity checks, one administrator-named timestamp-derived ordinary ALLOW insert, one request-owned exact-row callback, and one authorization revision advance. The callback validates current ETag before terminal conflict, resolves immutable target and narrowing, captures server-to-tool evidence under aggregate capacity, and writes revision 2 with the same timestamp and grant ID.
