...
func TestBrowserAudit(t *testing.T) {
	...
	require.NoError(t, json.NewEncoder(input).Encode(map[string]any{"version": 1, "scenario": "audit", "base_url": "http://" + harness.authority, "admin_bearer": harness.bearer}))
	require.NoError(t, input.Close())
	result, err := process.Wait()
	finished = true
	require.NoError(t, err, "audit browser scenario: %s", result.Stderr)
