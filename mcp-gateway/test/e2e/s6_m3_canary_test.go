...
func TestS6BrowserM3Canary(t *testing.T) {
	...
	require.NoError(t, json.NewEncoder(input).Encode(map[string]any{
		"version": 1, "scenario": "m3-canary", "base_url": "http://" + harness.authority,
	...
	finished = true
	require.NoError(t, waitErr, "M3 browser canary: %s", result.Stderr)
	...
	assert.Equal(t, "m3_complete", event.Event)
	...
	assert.Len(t, harness.results, 1, "M3 canary must own one Gateway lifecycle")
}
