...
func TestBrowserSessionLifecycleCanary(t *testing.T) {
	...
	require.NoError(t, json.NewEncoder(input).Encode(map[string]any{
		"version": 1, "scenario": "session-lifecycle-canary", "base_url": "http://" + harness.authority,
	...
	finished = true
	require.NoError(t, waitErr, "browser session lifecycle canary: %s", result.Stderr)
	...
	assert.Equal(t, "session_lifecycle_complete", event.Event)
	...
	assert.Len(t, harness.results, 1, "browser session lifecycle canary must own one Gateway lifecycle")
}
