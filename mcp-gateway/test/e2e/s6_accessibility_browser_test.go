...
func TestS6BrowserAccessibilityChanged(t *testing.T) {
	...
	require.NoError(t, json.NewEncoder(input).Encode(map[string]any{
		"version": 1, "scenario": "accessibility-changed", "base_url": "http://" + harness.authority,
	...
	finished = true
	require.NoError(t, waitErr, "accessibility browser scenario: %s", result.Stderr)
