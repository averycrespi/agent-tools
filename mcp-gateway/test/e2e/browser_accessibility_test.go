...
func TestBrowserAccessibility(t *testing.T) {
	...
	require.NoError(t, json.NewEncoder(input).Encode(map[string]any{
		"version": 1, "scenario": "accessibility-keyboard-responsive", "base_url": "http://" + harness.authority,
	...
	assert.Equal(t, "accessibility_keyboard_responsive_complete", event.Event)
