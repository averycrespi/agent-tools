...
func runCLIGrantRequestInputMatrix(t *testing.T) {
	...
	directApproved := runOnlineCLI(t, harness, bearerPath, true, "grant-request", "approve", fourth.ID, "--description", "Approved access", "--scope", "tool", "--target", "request-target.echo", "--yes", "--output", "json")
