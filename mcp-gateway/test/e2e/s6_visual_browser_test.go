...
func TestS6BrowserVisualChanged(t *testing.T) {
	...
	require.NoError(t, json.NewEncoder(input).Encode(map[string]any{
		"version": 1, "scenario": "visual-changed", "base_url": "http://" + harness.authority,
	...
	assert.Equal(t, "visual_changed_complete", event.Event)
