...
func TestS6CLIGrants(t *testing.T) {
	...
	grantBody := `{"principal_id":"` + principalID + `","effect":"deny","server_id":"` + contract.SyntheticServerID + `","upstream_name":"get_identity","constraint":{"equals":{"/count":1.0,"/nested~1name":true}},"expires_at":null}`
	invalidBody := `{"principal_id":"` + principalID + `","effect":"allow","server_id":"` + contract.SyntheticServerID + `","upstream_name":null,"constraint":{"equals":{"/bad":true}},"expires_at":null}`
