...
func (harness *gatewayHarness) CreateGrant(spec grantSpec) contract.Grant {
	...
	}{PrincipalID: spec.PrincipalID, Effect: string(spec.Effect), ServerID: spec.ServerID, UpstreamName: spec.UpstreamName, Constraint: spec.Constraint, ExpiresAt: spec.ExpiresAt})
	require.NoError(harness.t, err)
