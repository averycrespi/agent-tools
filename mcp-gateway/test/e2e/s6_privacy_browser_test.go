...
func TestS6BrowserPrivacyCanary(t *testing.T) {
	...
	finished = true
	require.NoError(t, waitErr, "privacy browser scenario: %s", result.Stderr)
	...
	assert.Equal(t, 18, event.Sinks)
	assert.Regexp(t, `^[0-9a-f]{64}$`, event.ScreenshotSHA256)

	harness.Stop(os.Interrupt)
	assert.Len(t, harness.results, 1, "browser privacy proof must own one Gateway lifecycle")
}
