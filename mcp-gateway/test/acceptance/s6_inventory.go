...
func s6Leaves(owner string, categories []S6LeafCategory) []S6PlannedLeaf {
	...
	for index, category := range categories {
		...
		if owner == "T1" {
			leaf.Argv = []string{"go", "-C", "mcp-gateway", "test", "-race", "-count=1", "-timeout=2m", "-run", "^Test(LegacyControlPlaneManifest|Acceptance(OwnerInventory|OwnerCommands|CapabilityCoverage|DocumentationCoverage))$", "./internal/contract", "./test/acceptance"}
