...
func ProfileCommands(profile Profile) ([]Command, error) {
	...
	case ProfileS4:
		return []Command{
			...
			{CheckName: "repeated_lifecycle_races", Name: "go", Arguments: []string{"-C", "mcp-gateway", "test", "-race", "-count=20", "-timeout=18m", "./internal/catalog", "./internal/downstream", "./internal/mcpingress", "./internal/composition"}, Criteria: []string{"AC-1", "AC-2", "AC-3", "AC-4", "AC-5"}, Evidence: []string{"s4-catalog-race", "s4-downstream-race", "s4-ingress-wire", "s4-composition-race", "s4-lifecycle-race"}, Artifacts: []string{"mcp-gateway/internal/catalog", "mcp-gateway/internal/downstream", "mcp-gateway/internal/mcpingress", "mcp-gateway/internal/composition"}},
