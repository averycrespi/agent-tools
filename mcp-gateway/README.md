...
Fallback closes the probe first and requires verified stdio process cleanup before opening a fresh legacy transport. Modern HTTP rejects all session state. A bounded legacy HTTP session is immutable and scoped to one runtime; loss or replacement closes that runtime rather than retrying or rerouting a call. No initialization cache, list-change subscription, multi-round-trip client feature, automatic reconnect, or pre-Streamable HTTP+SSE path exists. Production composition does not yet activate this seam.
