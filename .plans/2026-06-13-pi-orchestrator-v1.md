...
- Layering:
  - `po` owns workflows, workflow runs, steps, step runs, workflow artifacts, and workflow-run supervision. V1 workflow-run supervision is serial: at most one backing `pd` task run is active for a workflow run at a time.
...
- V1 includes adding the stable `pi-dispatcher/pkg/...` API that `po` needs. This should mirror the package boundary pattern already used by `pd` when it calls `worktree-manager/pkg/...` and `sandbox-manager/pkg/...`: a small public API for the operations the next layer needs, while implementation details remain internal.
...
- `sb` supports writable configured mounts with host/sandbox path identity. V1 should use one configured artifact parent directory that is mounted into the sandbox at the same absolute path; `po` creates per-workflow-run subdirectories under that parent.
...
- AC-5: The workflow supervisor executes step runs serially in deterministic workflow-file order, creating each backing `pd` task run through a stable `pd/pkg/...` API, recording both backing `pd_task_id` and `pd_run_id`, and never creating per-step worktrees.
...
- AC-9: `po ps`, `po status <run>`, `po wait <run>`, `po logs <run>`, `po stop <run>`, `po cleanup <run>`, and `po rm <run>` behave consistently with the corresponding `pd` concepts.
...
- AC-11: Dashboard routes use `po`-owned local auth, expose read-only APIs, and do not mutate workflow, worktree, artifact, or `pd` task state.
- AC-12: `po token rotate` rotates the `po` dashboard auth token without reusing or modifying `pd` auth state.
- AC-13: `po wait <run>` blocks until terminal state and exits non-zero unless the workflow run succeeded.
- AC-14: `po logs <run>` shows workflow supervisor logs and pointers to the underlying `pd` task logs for each step.
- AC-15: `po stop <run>` stops the workflow supervisor and the currently running backing `pd` task run, then records the workflow run as stopped.
- AC-16: Deterministic tests cover workflow loading/validation, input validation, graph execution ordering, shared-worktree behavior, artifact validation failure, wait exit behavior, stop behavior, terminal run cleanup, terminal run metadata removal, token rotation, and read-only dashboard API behavior.
- AC-17: Documentation explains the V1 workflow model, CLI commands, dashboard, state/artifact locations, and how `po` relates to `pd`.
...
- Store workflow artifacts outside the workflow worktree under a configured artifact parent directory that is already mounted into the sandbox at the same absolute path.
...
- Do not import `pd/internal/...` from `po`; expose the necessary dispatcher behavior through a new `pi-dispatcher/pkg/...` package as part of this plan.
...
- V1 supports explicit dependencies; cycles fail lint/run validation. When multiple steps are ready, the supervisor chooses deterministic workflow-file order and still runs only one step at a time.
...
po status <run>                        # show workflow run, steps, artifacts, backing pd task/run IDs
...
po cleanup <run>                       # remove terminal workflow worktree/artifacts
po cleanup --dry-run <run>             # show cleanup targets without mutating
po rm <run>                            # forget terminal workflow-run metadata/log references
po dashboard                           # read-only loopback web UI for workflow runs and steps
po token rotate                        # rotate po dashboard auth token
```
...
- `workflow_runs`: workflow run ID, workflow name/version snapshot, rendered repo/worktree metadata, artifact root, state, timestamps, supervisor metadata, supervisor log path, and terminal outcome.
- `step_runs`: workflow run ID, step ID, selected agent, dependency state, backing `pd_task_id`, backing `pd_run_id`, deterministic execution index, state, timestamps, and terminal outcome.
...
- Use `po`-owned local auth equivalent to `pd dashboard` because workflow prompts, repository paths, artifact paths, and `pd` log paths can be sensitive. Do not reuse `pd` auth tokens or cookies.
...
Dashboard auth is owned by `po`:

- token path: `$XDG_CONFIG_HOME/po/auth-token` or `~/.config/po/auth-token`;
- restrictive token file permissions;
- cookie name: `po-auth`;
- visiting the printed token URL sets an HttpOnly dashboard cookie;
- `po token rotate` replaces the token without printing the secret;
- running dashboard servers must be restarted after token rotation, matching `pd` behavior.
...
- Ensure the configured artifact parent directory is visible inside the sandbox at the same absolute path before starting any step.
- Ensure the workflow run artifact root exists as a per-run subdirectory under that configured parent and outside the workflow worktree.
- Select one ready step at a time in deterministic workflow-file order after dependencies have succeeded.
- Create the selected step's backing `pd` task run through the stable `pd/pkg/...` API.
- Persist each backing step's `pd_task_id` and `pd_run_id`.
...
- Ensure at most one backing `pd` task run is active for the workflow run at any time.
...
## Cleanup and Removal Semantics

Mirror the `pd` split between cleanup and metadata removal.

`po cleanup <run>` removes `po`-owned resources for terminal workflow runs:

- refuses non-terminal workflow runs;
- supports `--dry-run` to report targets without mutating;
- removes the workflow worktree through `wt` safety semantics;
- removes the workflow run's artifact directory;
- records cleanup status/errors in `po` metadata;
- preserves `po` workflow-run metadata, supervisor logs, and dashboard visibility;
- preserves backing `pd` task/run records, logs, event streams, and branches.

`po rm <run>` forgets terminal workflow-run metadata:

- refuses non-terminal workflow runs;
- removes `po` metadata rows and supervisor log references for the workflow run;
- removes stale `po` runtime/control files if any;
- preserves workflow worktree and artifacts unless `po cleanup` has already removed them;
- preserves backing `pd` task/run records, logs, event streams, and branches.

## `pd/pkg` Integration Requirements

V1 includes creating a stable dispatcher package API sufficient for `po` to create and observe backing task runs without importing `pd/internal/...` or scraping CLI output. This is analogous to how `pd` already uses public packages from `wt` and `sb`: `po` is one layer higher, so it needs a small public dispatcher surface rather than access to dispatcher internals.
...
- Start a task run with:
  ...
  - existing configured artifact parent path.
- Return durable dispatcher identifiers and metadata:
  - `pd_task_id`,
  - `pd_run_id`,
  - task/run state,
  - stdout/stderr log paths,
  - Pi event stream path when available,
  - control socket path only if needed by the stop implementation.
- Query task-run state and result metadata by `pd_task_id` and `pd_run_id`.
- Wait for a task run to reach terminal state.
- Stop a running task run.

The API should preserve `pd` ownership: `pd` still creates task/run records, supervises Pi execution, manages task-run logs, owns task-run lifecycle transitions, and decides how task IDs/run IDs map onto its SQLite schema. `po` stores both returned IDs and treats `pd` as the source of truth for backing task-run state.

## Artifact Handling

V1 uses one configured artifact parent directory that is mounted into the sandbox at the same absolute path. `po` does not request a new sandbox mount for each workflow run. Instead, for each workflow run it creates a subdirectory under the configured parent, for example:

```text
<artifact-parent>/<workflow-run-id>/
```

The artifact parent must be outside any workflow worktree but inside a host path that `sb` already mounts writable with host/sandbox path identity. `po run` must fail before starting steps if the artifact parent is not visible inside the sandbox at the same absolute path.

For each declared artifact, persist both the relative path and rendered absolute path. Prompts can safely refer to exact paths using `artifact_path` because every step sees the same per-run artifact root.
...
Artifact directories are `po`-owned resources. `po cleanup <run>` removes the terminal workflow run's artifact directory; `po rm <run>` only forgets metadata/log references and preserves artifacts unless cleanup has already removed them.
...
- `internal/auth`: `po`-owned dashboard token creation, auth middleware, and token rotation.
...
- `internal/artifact`: configured artifact parent validation, per-run artifact root creation, artifact path rendering, and required artifact validation.
- `internal/dispatcher`: adapter around the new `pd/pkg/...` API.
- `internal/cleanup`: conservative workflow worktree/artifact cleanup using `wt` safety semantics.
...
- Add support for accepting a caller-provided artifact parent path known to be mounted in the sandbox; V1 does not require per-task dynamic mount configuration.
- Keep the public API narrow and purpose-built for `po`, following the style of the existing `wt` and `sb` package APIs rather than exposing the whole dispatcher store.
...
- V5: focused CLI tests or integration tests prove:
  ...
  - serial execution keeps at most one backing `pd` task run active per workflow run,
  ...
  - `po cleanup --dry-run` reports terminal workflow worktree/artifact cleanup targets without mutating,
  - `po cleanup` removes terminal workflow worktree/artifacts using documented safety semantics while preserving `po` metadata and backing `pd` records/logs,
  - `po rm` removes terminal workflow-run metadata/log references while preserving workflow worktree/artifacts and backing `pd` records/logs,
  - `po token rotate` replaces the `po` auth token without printing it and does not touch `pd` auth state,
  - dashboard APIs return workflow-run summaries/details/log windows,
  - dashboard handlers reject unauthenticated requests and accept the `po` token/cookie flow,
...
- Risk: `pd/pkg/...` API shape expands too much. Mitigation: treat the API as a narrow layer-boundary package modeled after the existing `wt`/`sb` public packages: expose only create/observe/wait/stop capabilities needed by `po` V1 and keep `pd` lifecycle ownership intact.
- Risk: sharing one worktree across multiple steps creates cleanup ambiguity. Mitigation: workflow-run ownership is explicit; V1 executes steps serially, `po` owns the workflow worktree lifetime, and backing `pd` task runs must not clean it up independently.
- Risk: artifact paths differ between host and sandbox. Mitigation: use one configured artifact parent directory mounted with host/sandbox path identity, fail `po run` before starting steps if it is not visible in the sandbox, and test prompt-rendered artifact paths.
...
- Risk: command names collide semantically between workflow definitions and workflow runs. Mitigation: keep definition commands as `list/show/lint`; keep run inspection/control as `ps/status/wait/logs/stop/cleanup/rm`.
...
- Risk: dashboard auth couples `po` to `pd`. Mitigation: use `po`-owned equivalent auth with its own token path and cookie name; do not reuse `pd` auth tokens.

## Assumptions

- A separate `pi-orchestrator/` tool is the intended shape, matching the monorepo's independent-tool convention.
- `pd` can be refactored to expose the narrow package API required by `po` without changing existing `pd` CLI behavior.
