...
- `mcp-go` v0.57.0: use `req.GetArguments()` instead of `req.Params.Arguments` (typed as `any`, not `map[string]any`). Every tool schema sets `AdditionalProperties: false`, and server construction must include both `mcpserver.WithStrictInputSchemaDefault()` and `mcpserver.WithInputSchemaValidation()` so strictness is enforced before handlers run.
- Tool annotations: every tool sets one of the presets in `internal/tools/tools.go` (`annRead`, `annReadLocal`, `annIdempotent`, `annAdditive`, `annDestructive`). `TestEveryToolHasAnnotations` enforces this. `push` is `annDestructive` even without `force=true` — annotate by worst-case capability, not default behavior.
