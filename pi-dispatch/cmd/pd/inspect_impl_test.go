...
func TestShowStatusLabelsPiEventsAsRawArtifact(t *testing.T) {
	...
	run := store.Run{ID: "run-test", TaskID: task.ID, Attempt: 1, Status: store.StatusSucceeded, StartedAt: now, AgentOptionsJSON: `{"provider":"openai","model":"gpt-5","tools":["bash"],"system_prompt":"secret"}`, PiArgvJSON: `["pi","--mode","rpc","--provider","openai","--model","gpt-5","--tools","bash","--system-prompt","secret"]`, ControlSocketPath: "/sock", StdoutLogPath: "/stdout", StderrLogPath: "/stderr", PiEventsPath: "/events/pi-events.jsonl"}
