...
func TestShowStatusLabelsPiEventsAsRawArtifact(t *testing.T) {
	...
	run := store.Run{ID: "run-test", TaskID: task.ID, Attempt: 1, Status: store.StatusSucceeded, StartedAt: now, ControlSocketPath: "/sock", StdoutLogPath: "/stdout", StderrLogPath: "/stderr", PiEventsPath: "/events/pi-events.jsonl"}
