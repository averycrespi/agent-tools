...
func TestWaitJSONPrintsStatusView(t *testing.T) {
	...
	require.JSONEq(t, `{"task":{"id":"pd-test","status":"succeeded","repo_name":"repo","branch":"pd/test","worktree_path":"/wt","created_at":"2024-01-01T00:00:00Z","updated_at":"2024-01-01T00:00:00Z"},"run":{"id":"run-test","status":"succeeded","attempt":1,"control_socket_path":"/sock","stdout_log_path":"/stdout","stderr_log_path":"/stderr","pi_events_path":"/events"}}`, out)
}
...
func setupWaitTask(t *testing.T, status store.TaskStatus) (*store.Store, store.Task, store.Run) {
	...
	run := store.Run{ID: "run-test", TaskID: task.ID, Attempt: 1, SupervisorPID: 0, Status: status, StartedAt: now, ControlSocketPath: "/sock", StdoutLogPath: "/stdout", StderrLogPath: "/stderr", PiEventsPath: "/events"}
