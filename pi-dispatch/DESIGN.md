...
Worktree cleanup is task-level state. The task row records `worktree_cleanup_policy`, whether the worktree was created by this `pd run`, cleanup status/error, cleanup attempt time, and removal time. Cleanup is separate from task/run terminal status: cleanup failures do not alter exit code or `pd wait` success/failure behavior. Automatic cleanup is branch-preserving, non-forced, best-effort, and only targets worktrees created by the current task. Manual `pd cleanup <task-id>` removes task-owned external resources while preserving task history; `pd rm <task-id>` forgets pd metadata/logs/socket state only.
